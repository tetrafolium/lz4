...
#pragma GCC diagnostic ignored                                                 \
    "-Wmissing-braces" /* GCC bug 53119 : doesn't accept { 0 } as initializer  \
...
enum {
  ...
  DEC_BUFFER_BYTES =
      RING_BUFFER_BYTES + MESSAGE_MAX_BYTES // Intentionally larger to test
                                            // unsynchronized ring buffers
};

size_t write_int32(FILE *fp, int32_t i) { return fwrite(&i, sizeof(i), 1, fp); }
...
size_t read_int32(FILE *fp, int32_t *i) { return fread(i, sizeof(*i), 1, fp); }
...
#define CMPBUFSIZE (LZ4_COMPRESSBOUND(MESSAGE_MAX_BYTES))
    {
      ...
      const int cmpBytes = LZ4_compress_HC_continue(lz4Stream, inpPtr, cmpBuf,
                                                    inpBytes, CMPBUFSIZE);
