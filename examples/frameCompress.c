...
static compressResult_t compress_file_internal(FILE *f_in, FILE *f_out,
  ...
  /* write frame header */
  {
    size_t const headerSize =
        LZ4F_compressBegin(ctx, outBuff, outCapacity, &kPrefs);
  ...
  for (;;) {
    ...
    size_t const compressedSize =
        LZ4F_compressUpdate(ctx, outBuff, outCapacity, inBuff, readSize, NULL);
...
static compressResult_t compress_file(FILE *f_in, FILE *f_out) {
  ...
  size_t const outbufCapacity = LZ4F_compressBound(
      IN_CHUNK_SIZE, &kPrefs); /* large enough for any input <= IN_CHUNK_SIZE */
  ...
  if (!LZ4F_isError(ctxCreation) && src && outbuff) {
    result = compress_file_internal(f_in, f_out, ctx, src, IN_CHUNK_SIZE,
...
static int decompress_file_internal(FILE *f_in, FILE *f_out, LZ4F_dctx *dctx,
                                    ...
                                    size_t filled, size_t alreadyConsumed,
  ...
  while (ret != 0) {
    ...
    while (srcPtr < srcEnd && ret != 0) {
      ...
      ret = LZ4F_decompress(dctx, dst, &dstSize, srcPtr, &srcSize,
...
static int decompress_file_allocDst(FILE *f_in, FILE *f_out, LZ4F_dctx *dctx,
                                    void *src, size_t srcCapacity) {
  assert(f_in != NULL);
  assert(f_out != NULL);
  assert(dctx != NULL);
  assert(src != NULL);
  assert(srcCapacity >= LZ4F_HEADER_SIZE_MAX); /* ensure LZ4F_getFrameInfo() can
                                                  read enough data */
  ...
  int const decompressionResult = decompress_file_internal(
      f_in, f_out, dctx, src, srcCapacity, readSize - consumedSize,
      consumedSize, dst, dstCapacity);
...
int main(int argc, const char **argv) {
  ...
  /* compress */
  {
    ...
    printf("%s: %zu → %zu bytes, %.1f%%\n", inpFilename, (size_t)ret.size_in,
           (size_t)ret.size_out, /* might overflow is size_t is 32 bits and
                                    size_{in,out} > 4 GB */
