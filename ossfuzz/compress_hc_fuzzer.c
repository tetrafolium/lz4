...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  size_t const dstCapacity =
      FUZZ_getRange_from_uint32(dstCapacitySeed, 0, size);
  int const level =
      FUZZ_getRange_from_uint32(levelSeed, LZ4HC_CLEVEL_MIN, LZ4HC_CLEVEL_MAX);
  ...
  {
    int const dstSize =
        LZ4_compress_HC((const char *)data, dst, size, dstCapacity, level);
    if (dstSize > 0) {
  ...
  if (dstCapacity > 0) {
    ...
    int const dstSize = LZ4_compress_HC_destSize(
        state, (const char *)data, dst, &compressedSize, dstCapacity, level);
