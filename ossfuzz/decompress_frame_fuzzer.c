...
static void decompress(LZ4F_dctx *dctx, void *dst, size_t dstCapacity,
                       ...
                       size_t dictSize, const LZ4F_decompressOptions_t *opts) {
  LZ4F_resetDecompressionContext(dctx);
  if (dictSize == 0)
    LZ4F_decompress(dctx, dst, &dstCapacity, src, &srcSize, opts);
  else
    LZ4F_decompress_usingDict(dctx, dst, &dstCapacity, src, &srcSize, dict,
                              dictSize, opts);
}
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  size_t const dictSize =
      FUZZ_getRange_from_uint32(dictSizeSeed, 0, largeDictSize);
