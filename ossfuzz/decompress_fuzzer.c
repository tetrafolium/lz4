...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  size_t const dstCapacity =
      FUZZ_getRange_from_uint32(dstCapacitySeed, 0, 4 * size);
  ...
  /* No dictionary. */
  LZ4_decompress_safe_usingDict((char const *)data, dst, size, dstCapacity,
                                NULL, 0);
  /* Small external dictonary. */
  LZ4_decompress_safe_usingDict((char const *)data, dst, size, dstCapacity,
                                smallDict, smallDictSize);
  /* Large external dictionary. */
  LZ4_decompress_safe_usingDict((char const *)data, dst, size, dstCapacity,
                                largeDict, largeDictSize);
  /* Small prefix. */
  ...
  LZ4_decompress_safe_partial((char const *)data, dst, size, dstCapacity,
                              dstCapacity);
  free(dst);
  free(dict);
