...
LZ4F_frameInfo_t FUZZ_dataProducer_frameInfo(FUZZ_dataProducer_t *producer) {
  ...
  info.blockSizeID =
      FUZZ_dataProducer_range32(producer, LZ4F_max64KB - 1, LZ4F_max4MB);
  ...
  info.contentChecksumFlag = FUZZ_dataProducer_range32(
      producer, LZ4F_noContentChecksum, LZ4F_contentChecksumEnabled);
  info.blockChecksumFlag = FUZZ_dataProducer_range32(
      producer, LZ4F_noBlockChecksum, LZ4F_blockChecksumEnabled);
  return info;
}

LZ4F_preferences_t
FUZZ_dataProducer_preferences(FUZZ_dataProducer_t *producer) {
  LZ4F_preferences_t prefs = LZ4F_INIT_PREFERENCES;
  prefs.frameInfo = FUZZ_dataProducer_frameInfo(producer);
  prefs.compressionLevel =
      FUZZ_dataProducer_range32(producer, 0, LZ4HC_CLEVEL_MAX + 3) - 3;
