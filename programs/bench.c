...
static int LZ4_compressBlockNoStream(const struct compressionParameters *pThis,
...
static int LZ4_compressBlockStream(const struct compressionParameters *pThis,
                                   const char *src, char *dst, int srcSize,
                                   int dstSize) {
  int const acceleration = (pThis->cLevel < 0) ? -pThis->cLevel + 1 : 1;
...
static int LZ4_compressBlockStreamHC(const struct compressionParameters *pThis,
                                     const char *src, char *dst, int srcSize,
                                     int dstSize) {
...
#define DECOMP_MULT                                                            \
  1 /* test decompression DECOMP_MULT times longer than compression */
...
static U32 g_displayLevel =
    2; /* 0 : no display;   1: errors;   2 : + result + interaction + warnings;
...
static int BMK_benchMem(const void *srcBuffer, size_t srcSize,
  ...
  size_t const blockSize = (g_blockSize >= 32 ? g_blockSize : srcSize) +
                           (!srcSize) /* avoid div by 0 */;
  U32 const maxNbBlocks =
      (U32)((srcSize + (blockSize - 1)) / blockSize) + nbFiles;
  blockParam_t *const blockTable =
      (blockParam_t *)malloc(maxNbBlocks * sizeof(blockParam_t));
  size_t const maxCompressedSize =
      LZ4_compressBound((int)srcSize) +
  ...
  if (strlen(displayName) > 17)
    displayName +=
        strlen(displayName) - 17; /* can only display 17 characters */
  ...
  /* Init blockTable data */
  {
    ...
    for (nbBlocks = 0, fileNb = 0; fileNb < nbFiles; fileNb++) {
      ...
      for (; nbBlocks < blockEnd; nbBlocks++) {
        ...
        blockTable[nbBlocks].cRoom =
            (size_t)LZ4_compressBound((int)thisBlockSize);
  ...
  /* Bench */
  {
    ...
    U32 nbCompressionLoops =
        (U32)((5 MB) / (srcSize + 1)) +
        1; /* conservative initial compression speed estimate */
    U32 nbDecodeLoops = (U32)((200 MB) / (srcSize + 1)) +
                        1; /* conservative initial decode speed estimate */
...
#define NB_MARKS 4
    ...
    while (!cCompleted || !dCompleted) {
      ...
      DISPLAYLEVEL(2, "%2s-%-17.17s :%10u ->\r", marks[markNb], displayName,
                   (U32)srcSize);
      if (!cCompleted)
        memset(compressedBuffer, 0xE5,
               maxCompressedSize); /* warm up and erase result buffer */
      ...
      if (!cCompleted) { /* still some time to do compression tests */
        ...
        for (nbLoops = 0; nbLoops < nbCompressionLoops; nbLoops++) {
          ...
          for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
            ...
            if (LZ4_isError(rSize))
              EXM_THROW(1, "LZ4 compression failed");
        ...
        }
        {
          ...
          if (clockSpan > 0) {
            ...
            assert(fastestC > 0);
            nbCompressionLoops =
                (U32)(TIMELOOP_NANOSEC / fastestC) + 1; /* aim for ~1sec */
          } else {
      ...
      if (!dCompleted) {
        ...
        for (nbLoops = 0; nbLoops < nbDecodeLoops; nbLoops++) {
          ...
          for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
            int const regenSize = LZ4_decompress_safe_usingDict(
                ...
                (int)blockTable[blockNb].srcSize, dictBuf, dictSize);
            if (regenSize < 0) {
        ...
        }
        {
          ...
          if (clockSpan > 0) {
            ...
            assert(fastestD > 0);
            nbDecodeLoops =
                (U32)(TIMELOOP_NANOSEC / fastestD) + 1; /* aim for ~1sec */
          } else {
...
static void BMK_benchCLevel(void *srcBuffer, size_t benchedSize,
  ...
  if (g_displayLevel == 1 && !g_additionalParam)
    DISPLAY("bench %s %s: input %u bytes, %u seconds, %u KB blocks\n",
            LZ4_VERSION_STRING, LZ4_GIT_COMMIT_STRING, (U32)benchedSize,
            g_nbSeconds, (U32)(g_blockSize >> 10));
...
static void BMK_loadFiles(void *buffer, size_t bufferSize, size_t *fileSizes,
  ...
  for (n = 0; n < nbFiles; n++) {
    ...
    if (fileSize >
        bufferSize - pos) { /* buffer too small - stop after this file */
...
static void BMK_benchFileTable(const char **fileNamesTable, unsigned nbFiles,
                               int cLevel, int cLevelLast, const char *dictBuf,
                               int dictSize) {
  void *srcBuffer;
  size_t benchedSize;
...
static void BMK_syntheticTest(int cLevel, int cLevelLast,
                              double compressibility, const char *dictBuf,
                              int dictSize) {
  char name[20] = {0};
  ...
  BMK_benchCLevel(srcBuffer, benchedSize, name, cLevel, cLevelLast,
                  &benchedSize, 1, dictBuf, dictSize);
...
int BMK_benchFilesSeparately(const char **fileNamesTable, unsigned nbFiles,
  ...
  for (fileNb = 0; fileNb < nbFiles; fileNb++)
    BMK_benchFileTable(fileNamesTable + fileNb, 1, cLevel, cLevelLast, dictBuf,
                       dictSize);
...
int BMK_benchFiles(const char **fileNamesTable, unsigned nbFiles, int cLevel,
                   int cLevelLast, const char *dictFileName) {
  ...
  if (dictFileName) {
    ...
    if (!dictFileSize)
      EXM_THROW(25, "Dictionary error : could not stat dictionary file");
  ...
  if (nbFiles == 0)
    ...
    if (g_benchSeparately)
      ...
      BMK_benchFileTable(fileNamesTable, nbFiles, cLevel, cLevelLast, dictBuf,
                         dictSize);
  }
