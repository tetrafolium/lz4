...
#define RDG_RANDLENGTH                                                         \
  (((RDG_rand(seed) >> 7) & 7) ? (RDG_rand(seed) & 15)                         \
                               : (RDG_rand(seed) & 511) + 15)
void RDG_genBlock(void *buffer, size_t buffSize, size_t prefixSize,
                  double matchProba, litDistribTable lt, unsigned *seedPtr) {
...
void RDG_genOut(unsigned long long size, double matchProba, double litProba,
                unsigned seed) {
  ...
  /* Generate compressible data */
  while (total < size) {
    RDG_genBlock(buff, RDG_DICTSIZE + RDG_BLOCKSIZE, RDG_DICTSIZE, matchProba,
                 lt, &seed);
    if (size - total < RDG_BLOCKSIZE)
      genBlockSize = (size_t)(size - total);
    total += genBlockSize;
    fwrite(buff, 1, genBlockSize,
           stdout); /* should check potential write error */
    /* update dict */
