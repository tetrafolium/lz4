...
/*
  ...
  It is not part of LZ4 compression library, it is a user program of the LZ4
...
#define WELCOME_MESSAGE                                                        \
  "*** %s %i-bits v%s, by %s ***\n", COMPRESSOR_NAME,                          \
      (int)(sizeof(void *) * 8), LZ4_versionString(), AUTHOR
...
static unsigned displayLevel =
    2; /* 0 : no display ; 1: errors only ; 2 : downgradable normal ; 3 :
          non-downgradable normal; 4 : + information */
...
int LZ4IO_compressFilename_Legacy(
    const char *input_filename, const char *output_filename,
    int compressionlevel, const LZ4IO_prefs_t *prefs); /* hidden function */
...
static int usage_advanced(const char *exeName) {
  ...
  DISPLAY("--content-size : compressed frame includes original size "
          "(default:not present)\n");
  DISPLAY("--list FILE : lists information about .lz4 files (useful for files "
          "compressed with --content-size flag)\n");
  DISPLAY("--[no-]sparse  : sparse mode (default:enabled on file, disabled on "
          "stdout)\n");
  DISPLAY("--favor-decSpeed: compressed files decompress faster, but are less "
          "compressed \n");
  DISPLAY("--fast[=#]: switch to ultra fast compression level (default: %i)\n",
          1);
  DISPLAY("--best  : same as -%d\n", LZ4HC_CLEVEL_MAX);
  DISPLAY("Benchmark arguments : \n");
  DISPLAY(" -b#    : benchmark file(s), using # compression level (default : "
          "1) \n");
  DISPLAY(
...
static int usage_longhelp(const char *exeName) {
  ...
  DISPLAY("[output] can be left empty. In this case, it receives the following "
          "value :\n");
  DISPLAY(
  ...
  DISPLAY("               + for compression, output to filename%s \n",
          LZ4_EXTENSION);
  DISPLAY(
      "               + for decompression, output to filename without '%s'\n",
      LZ4_EXTENSION);
  DISPLAY("                    > if input filename has no '%s' extension : "
          "error \n",
          LZ4_EXTENSION);
  DISPLAY("\n");
  ...
  DISPLAY("-3 ... -%d => High compression; higher number == more compression "
          "but slower\n",
          LZ4HC_CLEVEL_MAX);
  DISPLAY("\n");
  ...
  DISPLAY("----------------\n");
  DISPLAY("1 : compress 'filename' fast, using default output name "
          "'filename.lz4'\n");
  DISPLAY("          %s filename\n", exeName);
  DISPLAY("\n");
  DISPLAY("Short arguments can be aggregated. For example :\n");
  DISPLAY("----------------------------------\n");
  DISPLAY("2 : compress 'filename' in high compression mode, overwrite output "
          "if exists\n");
 ...
 *  @return `om_decompress` if input filename has .lz4 extension and
 * `om_compress` otherwise.
 */
...
int main(int argc, const char **argv) {
  int i, cLevel = 1, cLevelLast = -10000, legacy_format = 0, forceStdout = 0,
         main_pause = 0, multiple_inputs = 0, all_arguments_are_files = 0,
         operationResult = 0;
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!all_arguments_are_files && argument[0] == '-') {
      ...
      if (argument[1] == '-') {
        ...
        if (!strcmp(argument, "--keep")) {
          LZ4IO_setRemoveSrcFile(prefs, 0); /* keep source file (default) */
          continue;
        }
      ...
      while (argument[1] != 0) {
        ...
        if (g_lz4c_legacy_commands) {
          ...
          if (!strcmp(argument, "c0")) {
            cLevel = 0; /* -c0 (fast compression) */
            argument++;
            continue;
          }
          if (!strcmp(argument, "c1")) {
            cLevel = 9; /* -c1 (high compression) */
            argument++;
            continue;
          }
          if (!strcmp(argument, "c2")) {
            cLevel = 12; /* -c2 (very high compression) */
            argument++;
            continue;
          }
          if (!strcmp(argument, "hc")) {
            cLevel = 12; /* -hc (very high compression) */
            argument++;
            continue;
          }
          if (!strcmp(argument, "y")) {
            LZ4IO_setOverwrite(
                prefs, 1); /* -y (answer 'yes' to overwrite permission) */
            continue;
          }
        }
        ...
        /* keep source file (default anyway, so useless) (for xz/lzma
         * compatibility) */
        case 'k':
          LZ4IO_setRemoveSrcFile(prefs, 0);
          break;
         ...
         * https://code.google.com/p/lz4/issues/detail?id=151 */
        case 'm':
          multiple_inputs = 1;
          break;
    ...
    /* Store first non-option arg in input_filename to preserve original cli
     * logic. */
    if (!input_filename) {
    ...
    DISPLAYLEVEL(1,
                 "Warning : %s won't be used ! Do you want multiple input "
                 "files (-m) ? \n",
                 argument);
  }
  ...
  if (multiple_inputs) {
    ...
    if (recursive) { /* at this stage, filenameTable is a list of paths, which
      ...
      extendedFileList =
          UTIL_createFileList(inFileNames, ifnIdx, &fileNamesBuf, &fileNamesNb);
      if (extendedFileList) {
        unsigned u;
  ...
  while ((!output_filename) && (multiple_inputs == 0)) {
    if (!IS_CONSOLE(stdout) && mode != om_list) {
       ...
       * Conversely, to ensure output will not become `stdout`, use `-m` command
       * flag */
      DISPLAYLEVEL(1, "Warning : using stdout as default output. Do not rely "
                      "on this behavior: use explicit `-c` instead ! \n");
      output_filename = stdoutmark;
      break;
    }
    if (mode == om_auto) { /* auto-determine compression or decompression, based
                              on file extension */
    ...
    if (mode ==
        om_decompress) { /* decompression to file (automatic name will work only
                            if input filename has correct format extension) */
      size_t outl;
  ...
  if (!strcmp(output_filename, stdoutmark) && IS_CONSOLE(stdout) &&
      !forceStdout) {
  ...
  if (mode == om_decompress) {
    if (multiple_inputs) {
      const char *const dec_extension =
          !strcmp(output_filename, stdoutmark) ? stdoutmark : LZ4_EXTENSION;
      assert(ifnIdx <= INT_MAX);
      operationResult = LZ4IO_decompressMultipleFilenames(
          inFileNames, (int)ifnIdx, dec_extension, prefs);
    } else {
      operationResult =
          DEFAULT_DECOMPRESSOR(input_filename, output_filename, prefs);
    }
    ...
    if (legacy_format) {
      ...
      if (multiple_inputs) {
        const char *const leg_extension =
            !strcmp(output_filename, stdoutmark) ? stdoutmark : LZ4_EXTENSION;
        LZ4IO_compressMultipleFilenames_Legacy(inFileNames, (int)ifnIdx,
                                               leg_extension, cLevel, prefs);
      } else {
      ...
      if (multiple_inputs) {
        const char *const comp_extension =
            !strcmp(output_filename, stdoutmark) ? stdoutmark : LZ4_EXTENSION;
        assert(ifnIdx <= INT_MAX);
        operationResult = LZ4IO_compressMultipleFilenames(
            inFileNames, (int)ifnIdx, comp_extension, cLevel, prefs);
      } else {
        operationResult =
            DEFAULT_COMPRESSOR(input_filename, output_filename, cLevel, prefs);
      }
    }
  }
