...
static int g_displayLevel =
    0; /* 0 : no display  ; 1: errors  ; 2 : + result + interaction + warnings ;
          3 : + progression; 4 : + information */
...
#define LZ4IO_STATIC_ASSERT(c)                                                 \
  {                                                                            \
    enum { LZ4IO_static_assert = 1 / (int)(!!(c)) };                           \
...
void LZ4IO_freePreferences(LZ4IO_prefs_t *prefs) { free(prefs); }
...
size_t LZ4IO_setBlockSizeID(LZ4IO_prefs_t *const prefs, unsigned bsid) {
  ...
  prefs->blockSize =
      blockSizeTable[(unsigned)prefs->blockSizeId - minBlockSizeID];
...
static FILE *LZ4IO_openDstFile(const char *dstFileName,
  ...
  if (!strcmp(dstFileName, stdoutmark)) {
    ...
    if (!prefs->overwrite &&
        strcmp(dstFileName,
               nulmark)) { /* Check if destination file already exists */
      ...
      if (testf != NULL) { /* dest exists, prompt for overwrite authorization */
        ...
        DISPLAY("%s already exists; do you wish to overwrite (y/N) ? ",
                dstFileName);
        {
          int ch = getchar();
          if ((ch != 'Y') && (ch != 'y')) {
...
static int LZ4IO_LZ4_compress(const char *src, char *dst, int srcSize,
                              int dstSize, int cLevel) {
  (void)cLevel;
...
int LZ4IO_compressFilename_Legacy(const char *input_filename,
  ...
  typedef int (*compress_f)(const char *src, char *dst, int srcSize,
                            int dstSize, int cLevel);
  compress_f const compressionFunction =
      (compressionlevel < 3) ? LZ4IO_LZ4_compress : LZ4_compress_HC;
  ...
  while (1) {
    ...
    /* Read Block */
    size_t const inSize =
        fread(in_buff, (size_t)1, (size_t)LEGACY_BLOCKSIZE, finput);
    if (inSize == 0)
      break;
    ...
    /* Compress Block */
    outSize = compressionFunction(in_buff, out_buff + 4, (int)inSize,
                                  outBuffSize, compressionlevel);
    assert(outSize >= 0);
    compressedfilesize += (unsigned long long)outSize + 4;
    DISPLAYUPDATE(2, "\rRead : %i MB  ==> %.2f%%   ", (int)(filesize >> 20),
    ...
    if (fwrite(out_buff, 1, (size_t)outSize + 4, foutput) !=
        (size_t)(outSize + 4)) {
  ...
  if (clockEnd == clockStart)
    clockEnd += 1;                 /* avoid division by zero (speed) */
  ...
  for (i = 0; i < ifntSize; i++) {
    ...
    if (!strcmp(suffix, stdoutmark)) {
      missed_files += LZ4IO_compressFilename_Legacy(
          inFileNamesTable[i], stdoutmark, compressionLevel, prefs);
      continue;
    }
    ...
    missed_files += LZ4IO_compressFilename_Legacy(
        inFileNamesTable[i], dstFileName, compressionLevel, prefs);
  }
...
static void *LZ4IO_createDict(size_t *dictSize,
  ...
  if (!circularBuf)
    EXM_THROW(25, "Allocation error : not enough memory for circular buffer");
  if (!dictFilename)
    EXM_THROW(25, "Dictionary error : no filename provided");

  dictFile = LZ4IO_openSrcFile(dictFilename);
  if (!dictFile)
    EXM_THROW(25, "Dictionary error : could not open dictionary file");
  ...
  do {
    readSize =
        fread(circularBuf + dictEnd, 1, circularBufSize - dictEnd, dictFile);
  ...
  *dictSize = dictLen;
  ...
  if (dictStart == 0) {
    /* We're in the simple case where the dict starts at the beginning of our
     * circular buffer. */
    dictBuf = circularBuf;
    circularBuf = NULL;
  } else {
    ...
    if (!dictBuf)
      EXM_THROW(25, "Allocation error : not enough memory");

    memcpy(dictBuf, circularBuf + dictStart, circularBufSize - dictStart);
    memcpy(dictBuf + circularBufSize - dictStart, circularBuf,
           dictLen - (circularBufSize - dictStart));
  }
...
static LZ4F_CDict *LZ4IO_createCDict(const LZ4IO_prefs_t *const prefs) {
  ...
  if (!prefs->useDictionary)
    return NULL;
  dictionaryBuffer =
      LZ4IO_createDict(&dictionarySize, prefs->dictionaryFilename);
  if (!dictionaryBuffer)
    EXM_THROW(25, "Dictionary error : could not create dictionary");
...
static cRess_t LZ4IO_createCResources(const LZ4IO_prefs_t *const prefs) {
  ...
  LZ4F_errorCode_t const errorCode =
      LZ4F_createCompressionContext(&(ress.ctx), LZ4F_VERSION);
  if (LZ4F_isError(errorCode))
    EXM_THROW(30, "Allocation error : can't create LZ4F context : %s",
  ...
  ress.dstBufferSize =
      LZ4F_compressFrameBound(blockSize, NULL); /* cover worst case */
...
static void LZ4IO_freeCResources(cRess_t ress) {
...

  {
    ...
    if (LZ4F_isError(errorCode))
      EXM_THROW(38, "Error : can't free LZ4F context resource : %s",
                LZ4F_getErrorName(errorCode));
  }
}
 ...
 */
static int LZ4IO_compressFilename_extRess(cRess_t ress, const char *srcFileName,
  ...
  prefs.frameInfo.blockChecksumFlag =
      (LZ4F_blockChecksum_t)io_prefs->blockChecksum;
  prefs.frameInfo.contentChecksumFlag =
      (LZ4F_contentChecksum_t)io_prefs->streamChecksum;
  ...
  if (ferror(srcFile))
    EXM_THROW(30, "Error reading %s ", srcFileName);
  filesize += readSize;

  /* single-block file */
  if (readSize < blockSize) {
    /* Compress in single pass */
    size_t const cSize = LZ4F_compressFrame_usingCDict(
        ctx, dstBuffer, dstBufferSize, srcBuffer, readSize, ress.cdict, &prefs);
    if (LZ4F_isError(cSize))
      EXM_THROW(31, "Compression failed : %s", LZ4F_getErrorName(cSize));
    compressedfilesize = cSize;
    DISPLAYUPDATE(2, "\rRead : %u MB   ==> %.2f%%   ",
                  (unsigned)(filesize >> 20),
                  (double)compressedfilesize / (filesize + !filesize) *
  ...
  /* multiple-blocks file */
  {
    ...
    size_t const headerSize = LZ4F_compressBegin_usingCDict(
        ctx, dstBuffer, dstBufferSize, ress.cdict, &prefs);
    ...
    while (readSize > 0) {
      size_t const outSize = LZ4F_compressUpdate(ctx, dstBuffer, dstBufferSize,
                                                 srcBuffer, readSize, NULL);
    ...
    /* End of Frame mark */
    {
      size_t const endSize =
          LZ4F_compressEnd(ctx, dstBuffer, dstBufferSize, NULL);
  ...
  DISPLAYLEVEL(2, "Compressed %llu bytes into %llu bytes ==> %.2f%%\n",
               ...
               (double)compressedfilesize /
                   (filesize + !filesize /* avoid division by zero */) * 100);

  return 0;
}

int LZ4IO_compressFilename(const char *srcFileName, const char *dstFileName,
                           int compressionLevel, const LZ4IO_prefs_t *prefs) {
  UTIL_time_t const timeStart = UTIL_getTime();
  clock_t const cpuStart = clock();
  cRess_t const ress = LZ4IO_createCResources(prefs);

  int const result = LZ4IO_compressFilename_extRess(
      ress, srcFileName, dstFileName, compressionLevel, prefs);
...
int LZ4IO_compressMultipleFilenames(const char **inFileNamesTable, int ifntSize,
  ...
  for (i = 0; i < ifntSize; i++) {
    size_t const ifnSize = strlen(inFileNamesTable[i]);
    if (!strcmp(suffix, stdoutmark)) {
      missed_files += LZ4IO_compressFilename_extRess(
          ress, inFileNamesTable[i], stdoutmark, compressionLevel, prefs);
      continue;
    }
    if (ofnSize <= ifnSize + suffixSize + 1) {
      free(dstFileName);
    ...
    missed_files += LZ4IO_compressFilename_extRess(
        ress, inFileNamesTable[i], dstFileName, compressionLevel, prefs);
  }
...
static unsigned LZ4IO_fwriteSparse(FILE *file, const void *buffer,
                                   size_t bufferSize, int sparseFileSupport,
  ...
  while (ptrT < bufferTEnd) {
    ...
    if (nb0T != seg0SizeT) { /* not all 0s */
      errno = 0;
      {
        ...
        if (seekResult)
          EXM_THROW(72, "Sparse skip error(%d): %s ; try --no-sparse",
  ...
  if (bufferSize &
      maskT) { /* size not multiple of sizeT : implies end of block */
    ...
    if (restPtr != restEnd) {
      ...
      storedSkips = 0;
      {
        size_t const sizeCheck =
            fwrite(restPtr, 1, (size_t)(restEnd - restPtr), file);
...
static unsigned g_magicRead =
    0; /* out-parameter of LZ4IO_decodeLegacyStream() */
static unsigned long long LZ4IO_decodeLegacyStream(FILE *finput, FILE *foutput,
  ...
  /* Allocate Memory */
  char *const in_buff =
      (char *)malloc((size_t)LZ4_compressBound(LEGACY_BLOCKSIZE));
  ...
  while (1) {
    ...
    /* Block Size */
    {
      ...
      if (sizeCheck != 4)
        EXM_THROW(52, "Read error : cannot access block size ");
    }
    ...
    /* Read Block */
    {
      ...
      if (sizeCheck != blockSize)
        EXM_THROW(52, "Read error : cannot access compressed block !");
    }

    /* Decode Block */
    {
      int const decodeSize = LZ4_decompress_safe(
          in_buff, out_buff, (int)blockSize, LEGACY_BLOCKSIZE);
      if (decodeSize < 0)
        EXM_THROW(53, "Decoding Failed ! Corrupted input detected !");
      streamSize += (unsigned long long)decodeSize;
      /* Write Block */
      storedSkips = LZ4IO_fwriteSparse(foutput, out_buff, (size_t)decodeSize,
                                       prefs->sparseFileSupport,
...
static void LZ4IO_loadDDict(dRess_t *ress, const LZ4IO_prefs_t *const prefs) {
  ...
  ress->dictBuffer =
      LZ4IO_createDict(&ress->dictBufferSize, prefs->dictionaryFilename);
  if (!ress->dictBuffer)
    EXM_THROW(25, "Dictionary error : could not create dictionary");
}
...
static dRess_t LZ4IO_createDResources(const LZ4IO_prefs_t *const prefs) {
  ...
  /* init */
  LZ4F_errorCode_t const errorCode =
      LZ4F_createDecompressionContext(&ress.dCtx, LZ4F_VERSION);
  if (LZ4F_isError(errorCode))
...
static void LZ4IO_freeDResources(dRess_t ress) {
  ...
  if (LZ4F_isError(errorCode))
    EXM_THROW(69, "Error : can't free LZ4F context resource : %s",
              LZ4F_getErrorName(errorCode));
  free(ress.srcBuffer);
  free(ress.dstBuffer);
...
LZ4IO_decompressLZ4F(dRess_t ress, FILE *const srcFile, FILE *const dstFile,
                     const LZ4IO_prefs_t *const prefs) {
  unsigned long long filesize = 0;
  ...
  /* Init feed with magic number (already consumed from FILE* sFile) */
  {
    ...
    nextToLoad = LZ4F_decompress_usingDict(
        ress.dCtx, ress.dstBuffer, &outSize, ress.srcBuffer, &inSize,
        ress.dictBuffer, ress.dictBufferSize, NULL);
    if (LZ4F_isError(nextToLoad))
      EXM_THROW(62, "Header error : %s", LZ4F_getErrorName(nextToLoad));
  }
  ...
  for (; nextToLoad;) {
    ...
    while ((pos < readSize) ||
      ...
      nextToLoad =
          LZ4F_decompress_usingDict(ress.dCtx, ress.dstBuffer, &decodedBytes,
                                    (char *)(ress.srcBuffer) + pos, &remaining,
                                    ress.dictBuffer, ress.dictBufferSize, NULL);
      if (LZ4F_isError(nextToLoad))
        EXM_THROW(66, "Decompression error : %s",
      ...
      if (decodedBytes) {
        if (!prefs->testMode)
          storedSkips =
              LZ4IO_fwriteSparse(dstFile, ress.dstBuffer, decodedBytes,
                                 prefs->sparseFileSupport, storedSkips);
  ...
  if (nextToLoad != 0)
    EXM_THROW(68, "Unfinished stream");
...
LZ4IO_passThrough(FILE *finput, FILE *foutput,
  ...
  while (readBytes) {
    ...
    total += readBytes;
    storedSkips = LZ4IO_fwriteSparse(foutput, buffer, readBytes,
                                     sparseFileSupport, storedSkips);
  }
  if (ferror(finput))
    EXM_THROW(51, "Read Error");
...
static unsigned long long selectDecoder(dRess_t ress, FILE *finput,
                                        FILE *foutput,
                                        const LZ4IO_prefs_t *const prefs) {
  unsigned char MNstore[MAGICNUMBER_SIZE];
  ...
  if (g_magicRead) { /* magic number already read from finput (see legacy
                        frame)*/
...
static int LZ4IO_decompressSrcFile(dRess_t ress, const char *input_filename,
                                   const char *output_filename,
                                   const LZ4IO_prefs_t *const prefs) {
...
static int LZ4IO_decompressDstFile(dRess_t ress, const char *input_filename,
                                   const char *output_filename,
                                   const LZ4IO_prefs_t *const prefs) {
  stat_t statbuf;
  int stat_result = 0;
  ...
  /* Copy owner, file permissions and modification time */
  if (stat_result != 0 && strcmp(output_filename, stdoutmark) &&
...
int LZ4IO_decompressFilename(const char *input_filename,
  ...
  int const missingFiles =
      LZ4IO_decompressDstFile(ress, input_filename, output_filename, prefs);
...
int LZ4IO_decompressMultipleFilenames(const char **inFileNamesTable,
  ...
  if (outFileName == NULL)
    EXM_THROW(70, "Memory allocation error");
  ...
  for (i = 0; i < ifntSize; i++) {
    ...
    if (!strcmp(suffix, stdoutmark)) {
      missingFiles +=
          LZ4IO_decompressSrcFile(ress, inFileNamesTable[i], stdoutmark, prefs);
      continue;
    }
    if (ofnSize <= ifnSize - suffixSize + 1) {
      free(outFileName);
      ...
      if (outFileName == NULL)
        EXM_THROW(71, "Memory allocation error");
    }
    if (ifnSize <= suffixSize || strcmp(suffixPtr, suffix) != 0) {
      DISPLAYLEVEL(1,
                   "File extension doesn't match expected LZ4_EXTENSION (%4s); "
                   "will not process file: %s\n",
    ...
    missingFiles +=
        LZ4IO_decompressDstFile(ress, inFileNamesTable[i], outFileName, prefs);
  }
...
typedef enum { lz4Frame = 0, legacyFrame, skippableFrame } LZ4IO_frameType_t;
...
static const char *LZ4IO_frameTypeNames[] = {"LZ4Frame", "LegacyFrame",
                                             "SkippableFrame"};
 ...
 */
static unsigned long long
LZ4IO_skipBlocksData(FILE *finput, const LZ4F_blockChecksum_t blockChecksumFlag,
  ...
  for (;;) {
    ...
    totalBlocksSize += LZ4F_BLOCK_HEADER_SIZE;
    {
      ...
      const unsigned long nextBlock =
          nextCBlockSize + (blockChecksumFlag * LZ4F_BLOCK_CHECKSUM_SIZE);
      if (nextCBlockSize == 0) {
        /* Reached EndMark */
...
const char *LZ4IO_blockTypeID(LZ4F_blockSizeID_t sizeID,
                              LZ4F_blockMode_t blockMode, char buffer[4]) {
  buffer[0] = 'B';
  assert(sizeID >= 4);
  assert(sizeID <= 7);
 ...
 * - TODO :
 *  + report nb of blocks, hence max. possible decompressed size (when not
 * reported in header)
 */
static LZ4IO_infoResult
LZ4IO_getCompressedFileInfo(LZ4IO_cFileInfo_t *cfinfo,
  ...
  while (!feof(finput)) {
    ...
    case LZ4IO_MAGICNUMBER:
      ...
      /* Get frame info */
      {
        const size_t readBytes =
            fread(buffer + MAGICNUMBER_SIZE, 1,
                  LZ4F_HEADER_SIZE_MIN - MAGICNUMBER_SIZE, finput);
        if (!readBytes || ferror(finput))
          EXM_THROW(71, "Error reading %s", input_filename);
      }
      {
        ...
        if (hSize > (LZ4F_HEADER_SIZE_MIN + MAGICNUMBER_SIZE)) {
          /* We've already read LZ4F_HEADER_SIZE_MIN so read any extra until
           * hSize*/
          const size_t readBytes = fread(buffer + LZ4F_HEADER_SIZE_MIN, 1,
                                         hSize - LZ4F_HEADER_SIZE_MIN, finput);
          if (!readBytes || ferror(finput))
            EXM_THROW(72, "Error reading %s", input_filename);
        }
        /* Create decompression context */
        {
          ...
          if (LZ4F_isError(
            ...
            break;
          {
            unsigned const frameInfoError = LZ4F_isError(LZ4F_getFrameInfo(
                dctx, &frameInfo.lz4FrameInfo, buffer, &hSize));
            LZ4F_freeDecompressionContext(dctx);
            if (frameInfoError)
              break;
            if ((cfinfo->frameSummary.lz4FrameInfo.blockSizeID !=
                     frameInfo.lz4FrameInfo.blockSizeID ||
                 cfinfo->frameSummary.lz4FrameInfo.blockMode !=
                     frameInfo.lz4FrameInfo.blockMode) &&
                cfinfo->frameCount != 0)
              cfinfo->eqBlockTypes = 0;
            {
              const unsigned long long totalBlocksSize = LZ4IO_skipBlocksData(
                  finput, frameInfo.lz4FrameInfo.blockChecksumFlag,
              ...
              if (totalBlocksSize) {
                ...
                LZ4IO_blockTypeID(frameInfo.lz4FrameInfo.blockSizeID,
                                  ...
                                  bTypeBuffer);
                DISPLAYLEVEL(
                    3, "    %6llu %14s %5s %8s", cfinfo->frameCount + 1,
                ...
                if (frameInfo.lz4FrameInfo.contentSize) {
                  ...
                  /* Now we've consumed frameInfo we can use it to store the
                   * total contentSize */
                  frameInfo.lz4FrameInfo.contentSize +=
                      cfinfo->frameSummary.lz4FrameInfo.contentSize;
                } else {
    ...
    case LEGACY_MAGICNUMBER:
      ...
      if (cfinfo->frameSummary.frameType != legacyFrame &&
          cfinfo->frameCount != 0)
        cfinfo->eqFrameTypes = 0;
      cfinfo->eqBlockTypes = 0;
      cfinfo->allContentSize = 0;
    ...
    case LZ4IO_SKIPPABLE0:
      ...
      if (cfinfo->frameSummary.frameType != skippableFrame &&
          cfinfo->frameCount != 0)
        cfinfo->eqFrameTypes = 0;
      cfinfo->eqBlockTypes = 0;
      cfinfo->allContentSize = 0;
...
int LZ4IO_displayCompressedFilesInfo(const char **inFileNames, size_t ifnIdx) {
  ...
  for (; idx < ifnIdx; idx++) {
    ...
    DISPLAYLEVEL(3, "%s(%llu/%llu)\n", cfinfo.fileName,
                 (unsigned long long)idx + 1, (unsigned long long)ifnIdx);
    DISPLAYLEVEL(3, "    %6s %14s %5s %8s %20s %20s %9s\n", "Frame", "Type",
                 "Block", "Checksum", "Compressed", "Uncompressed", "Ratio") {
      LZ4IO_infoResult const op_result =
          LZ4IO_getCompressedFileInfo(&cfinfo, inFileNames[idx]);
    ...
    if (g_displayLevel < 3) {
      /* Display Summary */
      {
        char buffers[3][10];
        DISPLAYOUT(
            "%10llu %14s %5s %11s %13s ", cfinfo.frameCount,
            cfinfo.eqFrameTypes
                ? LZ4IO_frameTypeNames[cfinfo.frameSummary.frameType]
                : "-",
            cfinfo.eqBlockTypes
                ? LZ4IO_blockTypeID(
                      cfinfo.frameSummary.lz4FrameInfo.blockSizeID,
            ...
            cfinfo.allContentSize
                ? LZ4IO_toHuman(
                      (long double)cfinfo.frameSummary.lz4FrameInfo.contentSize,
                      buffers[2])
                : "-");
        if (cfinfo.allContentSize) {
          double const ratio = (double)cfinfo.fileSize /
                               cfinfo.frameSummary.lz4FrameInfo.contentSize *
                               100;
