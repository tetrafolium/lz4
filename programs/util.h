...
UTIL_STATIC int UTIL_setFileStat(const char *filename, stat_t *statbuf) {
...

  {
    ...
    res += utimensat(AT_FDCWD, filename, timebuf,
                     0); /* set access and modification times */
#endif
  }
...
UTIL_STATIC int UTIL_prepareFileList(const char *dirName, char **bufStart,
                                     size_t *pos, char **bufEnd) {
  char *path;
...
UTIL_STATIC int UTIL_prepareFileList(const char *dirName, char **bufStart,
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (UTIL_isDirectory(path)) {
      ...
      if (*bufStart + *pos + pathLength >= *bufEnd) {
        size_t const newListSize =
            (size_t)(*bufEnd - *bufStart) + LIST_SIZE_INCREASE;
        *bufStart = (char *)UTIL_realloc(*bufStart, newListSize);
        *bufEnd = *bufStart + newListSize;
        if (*bufStart == NULL) {
          free(path);
...
UTIL_STATIC const char **UTIL_createFileList(const char **inputNames,
  ...
  for (i = 0, pos = 0, nbFiles = 0; i < inputNamesNb; i++) {
    if (!UTIL_isDirectory(inputNames[i])) {
      ...
      nbFiles +=
          (unsigned)UTIL_prepareFileList(inputNames[i], &buf, &pos, &bufend);
      if (buf == NULL)
        return NULL;
      assert(bufend > buf);
  ...
  fileTable =
      (const char **)malloc(((size_t)nbFiles + 1) * sizeof(const char *));
  if (!fileTable) {
    free(buf);
    return NULL;
  }
