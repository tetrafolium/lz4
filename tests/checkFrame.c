...
static int createCResources(cRess_t *ress) {
  ...
  if (LZ4F_isError(
          LZ4F_createDecompressionContext(&(ress->ctx), LZ4F_VERSION))) {
    free(ress->srcBuffer);
    free(ress->dstBuffer);
...
int frameCheck(cRess_t ress, FILE *const srcFile, unsigned bsid,
               size_t blockSize) {
  ...
  for (;;) {
    ...
    while (pos < readSize) { /* still to read */
      ...
      if (!nextToLoad) {
        ...
        nextToLoad = LZ4F_getFrameInfo(
            ress.ctx, &frameInfo, (char *)(ress.srcBuffer) + pos, &remaining);
        ...
        nextToLoad =
            LZ4F_decompress(ress.ctx, ress.dstBuffer, &decodedBytes,
                            (char *)(ress.srcBuffer) + pos, &remaining, NULL);
        if (LZ4F_isError(nextToLoad))
          EXM_THROW(24, "Decompression error : %s",
                    LZ4F_getErrorName(nextToLoad));
        pos += remaining;
      }
      decodedBytes = ress.dstBufferSize;
      ...
      pos += remaining;
      if (!partialBlock) {
        /* detect small block due to end of frame; the final 4-byte frame
         * checksum could be left in the buffer */
