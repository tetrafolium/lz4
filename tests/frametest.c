...
#define DISPLAYUPDATE(l, ...)                                                  \
  if (displayLevel >= l) {                                                     \
    if ((FUZ_GetClockSpan(g_clockTime) > refreshRate) ||                       \
        (displayLevel >= 4)) {                                                 \
      g_clockTime = clock();                                                   \
      DISPLAY(__VA_ARGS__);                                                    \
      if (displayLevel >= 4)                                                   \
...
#define FUZ_RANDLENGTH                                                         \
  ((FUZ_rand(seed) & 3) ? (FUZ_rand(seed) % 15) : (FUZ_rand(seed) % 510) + 15)
...
#define COMPRESSIBLE_NOISE_LENGTH (2 MB)
  ...
  size_t const cBuffSize =
      LZ4F_compressFrameBound(COMPRESSIBLE_NOISE_LENGTH, NULL);
  ...
  CHECK_V(cSize, LZ4F_compressFrame(compressedBuffer,
                                    LZ4F_compressFrameBound(testSize, NULL),
                                    CNBuffer, testSize, NULL));
  ...
  assert(cSize >= LZ4F_MIN_SIZE_TO_KNOW_HEADER_LENGTH);
  {
    ...
    size_t const fhs =
        LZ4F_headerSize(compressedBuffer, LZ4F_MIN_SIZE_TO_KNOW_HEADER_LENGTH);
    size_t avail_in = fhs;
    CHECK(fhs);
    CHECK(LZ4F_getFrameInfo(dCtx, &frame_info, compressedBuffer, &avail_in));
    if (avail_in != fhs)
      goto _output_error; /* must consume all, since header size is supposed to
                             be exact */
  }
  ...
  DISPLAYLEVEL(3, "LZ4F_compressFrame, using fast level -3 : ");
  {
    ...
    CHECK_V(cSize, LZ4F_compressFrame(compressedBuffer,
                                      LZ4F_compressFrameBound(testSize, NULL),
                                      CNBuffer, testSize, &fastCompressPrefs));
  ...
  DISPLAYLEVEL(3, "Decompression test : \n");
  {
    ...
    CHECK(LZ4F_decompress(dCtx, decodedBuffer, &decodedBufferSize,
                          compressedBuffer, &compressedBufferSize, NULL));
    {
    ...
    DISPLAYLEVEL(3, "Reusing decompression context \n");
    {
      ...
      CHECK_V(decResult,
              LZ4F_decompress(dCtx, op, &oSize, cBuff, &iSize, NULL));
...

    {
      ...
      DISPLAYLEVEL(3, "LZ4F_getFrameInfo on zero-size input : ");
      {
        ...
        if (LZ4F_getErrorCode(fiError) != LZ4F_ERROR_frameHeader_incomplete) {
          DISPLAYLEVEL(
              3, "incorrect error : %s != ERROR_frameHeader_incomplete \n",
    ...
    assert(cSize > 48);
    memcpy(decodedBuffer, (char *)compressedBuffer + 16,
           32); /* save correct data */
    memcpy((char *)compressedBuffer + 16, (const char *)decodedBuffer + 32,
           32); /* insert noise */
    {
      size_t dbSize = COMPRESSIBLE_NOISE_LENGTH;
      size_t cbSize = cSize;
      size_t const decompressError = LZ4F_decompress(
          dCtx, decodedBuffer, &dbSize, compressedBuffer, &cbSize, NULL);
      if (!LZ4F_isError(decompressError))
        goto _output_error;
      DISPLAYLEVEL(3, "error detected : %s \n",
                   LZ4F_getErrorName(decompressError));
    }
    memcpy((char *)compressedBuffer + 16, decodedBuffer,
           32); /* restore correct data */

    DISPLAYLEVEL(
        3, "Reset decompression context, since it's left in error state \n");
  ...
  CHECK_V(cSize, LZ4F_compressFrame(compressedBuffer,
                                    LZ4F_compressFrameBound(testSize, &prefs),
                                    CNBuffer, testSize, &prefs));
  ...
  prefs.frameInfo.contentChecksumFlag = LZ4F_contentChecksumEnabled;
  {
    ...
    CHECK_V(cSize, LZ4F_compressFrame(compressedBuffer, dstCapacity, CNBuffer,
                                      testSize, &prefs));
    DISPLAYLEVEL(3, "Compressed %u bytes into a %u bytes frame \n",
                 (U32)testSize, (U32)cSize);
  }
  ...
  DISPLAYLEVEL(3, "Decompress with block checksum : ");
  {
    ...
    CHECK(LZ4F_createDecompressionContext(&dctx, LZ4F_VERSION));
    CHECK(LZ4F_decompress(dctx, decodedBuffer, &decodedSize, compressedBuffer,
                          &iSize, NULL));
  ...
  /* frame content size tests */
  {
    ...
    op += cErr;
    CHECK_V(cErr,
            LZ4F_compressUpdate(cctx, op, LZ4F_compressBound(testSize, &prefs),
                                CNBuffer, testSize, NULL));
    op += cErr;
  ...
  /* Dictionary compression test */
  {
    ...
    DISPLAYLEVEL(3, "LZ4F_decompress_usingDict : ");
    {
      ...
      CHECK(LZ4F_createDecompressionContext(&dctx, LZ4F_VERSION));
      CHECK(LZ4F_decompress_usingDict(dctx, decodedBuffer, &decodedSize,
                                      compressedBuffer, &compressedSize,
                                      CNBuffer, dictSize, NULL));
      if (compressedSize != cSizeWithDict)
        goto _output_error;
      if (decodedSize != dictSize)
        goto _output_error;
      {
                 ...
                 "LZ4F_compressFrame_usingCDict, with dict, negative level : ");
    {
      ...
      CHECK_V(cSizeLevelMax, LZ4F_compressFrame_usingCDict(
                                 cctx, compressedBuffer, dstCapacity, CNBuffer,
    ...
    DISPLAYLEVEL(
        3, "LZ4F_compressFrame_usingCDict, multiple independent blocks : ");
    {
      size_t cSizeIndep;
      size_t const inSize = dictSize * 3;
      size_t const outCapacity = LZ4F_compressFrameBound(inSize, NULL);
      ...
      DISPLAYLEVEL(
          3, "LZ4F_decompress_usingDict on multiple independent blocks : ");
      {
        LZ4F_dctx *dctx;
        size_t decodedSize = COMPRESSIBLE_NOISE_LENGTH;
...
size_t test_lz4f_decompression_wBuffers(
    const void *cSrc, size_t cSize, void *dst, size_t dstCapacity,
    o_scenario_e o_scenario, const void *srcRef, size_t decompressedSize,
    U64 crcOrig, U32 *const randState, LZ4F_dctx *const dCtx, U32 seed,
    U32 testNb, int findErrorPos) {
  ...
  while (ip < iend) {
    ...
    /* read data from byte-exact buffer to catch out-of-bound reads */
    {
      ...
      const void *const tmpip =
          (FUZ_rand(randState) & (iSize == 0)) ? NULL : iBuffer;
      assert(iBuffer != NULL);
...
size_t test_lz4f_decompression(const void *cSrc, size_t cSize,
                               ...
                               U64 crcOrig, U32 *const randState,
                               LZ4F_dctx *const dCtx, U32 seed, U32 testNb,
                               int findErrorPos) {
  o_scenario_e const o_scenario = (o_scenario_e)(
      FUZ_rand(randState) %
  ...
  result = test_lz4f_decompression_wBuffers(
      cSrc, cSize, dstBuffer, dstCapacity, o_scenario, srcRef, decompressedSize,
...
int fuzzerTests(U32 seed, unsigned nbTests, unsigned startTest,
                double compressibility, U32 duration_s) {
  unsigned testNb = 0;
  size_t const CNBufferLength =
      9 MB; /* needs to be > 2x4MB to test large blocks */
  void *CNBuffer = NULL;
  size_t const compressedBufferSize =
      LZ4F_compressFrameBound(CNBufferLength, NULL) +
  ...
  /* Create buffers */
  {
    size_t const creationStatus =
        LZ4F_createDecompressionContext(&dCtx, LZ4F_VERSION);
    CHECK(LZ4F_isError(creationStatus), "Allocation failed (error %i)",
          (int)creationStatus);
  }
  {
    size_t const creationStatus =
        LZ4F_createDecompressionContext(&dCtxNoise, LZ4F_VERSION);
    CHECK(LZ4F_isError(creationStatus), "Allocation failed (error %i)",
          (int)creationStatus);
  }
  {
  ...
  decodedBuffer =
      calloc(1, CNBufferLength); /* calloc avoids decodedBuffer being considered
                                    "garbage" by scan-build */
  CHECK(decodedBuffer == NULL, "decodedBuffer Allocation failed");
  FUZ_fillCompressibleNoiseBuffer(CNBuffer, CNBufferLength, compressibility,
                                  &coreRand);
  ...
  for (; (testNb < nbTests) || (clockDuration > FUZ_GetClockSpan(startClock));
       testNb++) {
    U32 randState = coreRand ^ prime1;
    unsigned const srcBits =
        (FUZ_rand(&randState) % (FUZ_highbit((U32)(CNBufferLength - 1)) - 1)) +
        1;
    ...
    prefs.frameInfo.blockSizeID =
        (LZ4F_blockSizeID_t)(4 + (FUZ_rand(&randState) & 3));
    prefs.frameInfo.blockChecksumFlag =
        (LZ4F_blockChecksum_t)(FUZ_rand(&randState) & 1);
    prefs.frameInfo.contentChecksumFlag =
        (LZ4F_contentChecksum_t)(FUZ_rand(&randState) & 1);
    prefs.frameInfo.contentSize =
        ((FUZ_rand(&randState) & 0xF) == 1) ? srcSize : 0;
    ...
    if ((FUZ_rand(&randState) & 0xFFF) == 0) {
      ...
      FUZ_writeLE32(op,
                    LZ4F_MAGIC_SKIPPABLE_START + (FUZ_rand(&randState) & 15));
      FUZ_writeLE32(op + 4, (U32)srcSize);
      cSize = srcSize + 8;

    } else if ((FUZ_rand(&randState) & 0xF) ==
               2) { /* single pass compression (simple) */
      cSize = LZ4F_compressFrame(compressedBuffer,
                                 LZ4F_compressFrameBound(srcSize, prefsPtr),
                                 srcStart, srcSize, prefsPtr);
      CHECK(LZ4F_isError(cSize), "LZ4F_compressFrame failed : error %i (%s)",
            (int)cSize, LZ4F_getErrorName(cSize));
      ...
      BYTE *const oend =
          ...
          (neverFlush
               ? LZ4F_compressFrameBound(srcSize, prefsPtr)
               : compressedBufferSize); /* when flushes are possible, can't
                                           guarantee a max compressed size */
      unsigned const maxBits = FUZ_highbit((U32)srcSize);
      LZ4F_compressOptions_t cOptions;
      memset(&cOptions, 0, sizeof(cOptions));
      {
        size_t const fhSize =
            LZ4F_compressBegin(cCtx, op, (size_t)(oend - op), prefsPtr);
      ...
      while (ip < iend) {
        ...
        flushedSize =
            LZ4F_compressUpdate(cCtx, op, oSize, ip, iSize, &cOptions);
        ...
        {
          unsigned const forceFlush =
              neverFlush ? 0 : ((FUZ_rand(&randState) & 3) == 1);
          if (forceFlush) {
            size_t const flushSize =
                LZ4F_flush(cCtx, op, (size_t)(oend - op), &cOptions);
            DISPLAYLEVEL(6, "flushing %u bytes \n", (unsigned)flushSize);
            CHECK(LZ4F_isError(flushSize), "Compression failed (error %i)",
                  (int)flushSize);
            op += flushSize;
    ...
    {
      size_t result = test_lz4f_decompression(
          compressedBuffer, cSize, srcStart, srcSize, crcOrig, &randState, dCtx,
          seed, testNb, 1 /*findError*/);
      CHECK(LZ4F_isError(result),
            "multi-segment decompression failed (error %i => %s)", (int)result,
    ...
    /* insert noise into src */
    {
      ...
      for (;;) {
        ...
        /* add noise */
        {
          ...
          memcpy((BYTE *)compressedBuffer + pos,
    ...
    test_lz4f_decompression(compressedBuffer, cSize, srcStart, srcSize, crcOrig,
                            &randState, dCtxNoise, seed, testNb,
                            0 /*don't search error Pos*/);
    /* note : we don't analyze result here : it probably failed, which is
     * expected. The sole purpose is to catch potential out-of-bound reads and
     * writes. */
    LZ4F_resetDecompressionContext(
        dCtxNoise); /* context must be reset after an error */
#endif
...
int main(int argc, const char **argv) {
  ...
  if (testNb == 0)
    result = basicTests(seed, ((double)proba) / 100);
  if (result)
    return 1;
