...
#define WELCOME_MESSAGE                                                        \
  "*** %s v%s %i-bits, by %s ***\n", PROGRAM_DESCRIPTION, LZ4_VERSION_STRING,  \
...
static void BMK_setPause(void) { g_pause = 1; }
...
static int local_LZ4_compress_default_large(const char *in, char *out,
                                            int inSize) {
  return LZ4_compress_default(in, out, inSize, LZ4_compressBound(inSize));
}

static int local_LZ4_compress_default_small(const char *in, char *out,
                                            int inSize) {
...
static int local_LZ4_compress_fast_extState0(const char *in, char *out,
                                             int inSize) {
  return LZ4_compress_fast_extState(&LZ4_stream, in, out, inSize,
                                    LZ4_compressBound(inSize), 0);
}

static int local_LZ4_compress_fast_continue0(const char *in, char *out,
                                             int inSize) {
  return LZ4_compress_fast_continue(&LZ4_stream, in, out, inSize,
                                    LZ4_compressBound(inSize), 0);
}
...
extern int LZ4_compress_forceExtDict(LZ4_stream_t *LZ4_stream,
...
static int local_LZ4_compress_HC_extStateHC(const char *in, char *out,
                                            int inSize) {
  return LZ4_compress_HC_extStateHC(&LZ4_streamHC, in, out, inSize,
                                    LZ4_compressBound(inSize), 9);
}

static int local_LZ4_compress_HC_continue(const char *in, char *out,
                                          int inSize) {
  return LZ4_compress_HC_continue(&LZ4_streamHC, in, out, inSize,
                                  LZ4_compressBound(inSize));
}

/* decompression functions */
static int local_LZ4_decompress_fast(const char *in, char *out, int inSize,
                                     int outSize) {
  (void)inSize;
...
static int local_LZ4_decompress_fast_usingDict_prefix(const char *in, char *out,
...
static int local_LZ4_decompress_fast_usingExtDict(const char *in, char *out,
                                                  int inSize, int outSize) {
  (void)inSize;
...
static int local_LZ4_decompress_safe_withPrefix64k(const char *in, char *out,
                                                   int inSize, int outSize) {
  LZ4_decompress_safe_withPrefix64k(in, out, inSize, outSize);
  return outSize;
}

static int local_LZ4_decompress_safe_usingDict(const char *in, char *out,
                                               int inSize, int outSize) {
...
extern int LZ4_decompress_safe_forceExtDict(const char *in, char *out,
                                            int inSize, int outSize,
...
static int local_LZ4_decompress_safe_forceExtDict(const char *in, char *out,
                                                  int inSize, int outSize) {
  (void)inSize;
  LZ4_decompress_safe_forceExtDict(in, out, inSize, outSize, out - 65536,
                                   65536);
  return outSize;
}
#endif

static int local_LZ4_decompress_safe_partial(const char *in, char *out,
                                             int inSize, int outSize) {
  int result =
      LZ4_decompress_safe_partial(in, out, inSize, outSize - 5, outSize);
  if (result < 0)
    return result;
  return outSize;
}
...
static int local_LZ4F_compressFrame(const char *in, char *out, int inSize) {
  assert(inSize >= 0);
  return (int)LZ4F_compressFrame(out,
                                 LZ4F_compressFrameBound((size_t)inSize, NULL),
                                 in, (size_t)inSize, NULL);
}

static LZ4F_decompressionContext_t g_dCtx;

static int local_LZ4F_decompress(const char *in, char *out, int inSize,
                                 int outSize) {
...
static int local_LZ4F_decompress_followHint(const char *src, char *dst,
  ...
  for (;;) {
    size_t const sizeHint = LZ4F_decompress(g_dCtx, dst + outPos, &outRemaining,
                                            src + inPos, &inSize, NULL);
...
static int local_LZ4F_decompress_noHint(const char *src, char *dst, int srcSize,
                                        int dstSize) {
  size_t totalInSize = (size_t)srcSize;
  size_t maxOutSize = (size_t)dstSize;
...
int fullSpeedBench(const char **fileNamesTable, int nbFiles) {
  ...
  while (fileIdx < nbFiles) {
    ...
    size_t benchedSize =
        BMK_findMaxMem(inFileSize * 2) / 2; /* because 2 buffers */
    int nbChunks;
    ...
    chunkP = (struct chunkParameters *)malloc(
        ((benchedSize / (size_t)g_chunkSize) + 1) *
        sizeof(struct chunkParameters));
    orig_buff = (char *)malloc(benchedSize);
    nbChunks =
        (int)((benchedSize + (size_t)g_chunkSize - 1) / (size_t)g_chunkSize);
    ...
    /* Bench */
    {
      ...
      for (cAlgNb = 0;
           (cAlgNb <= NB_COMPRESSION_ALGORITHMS) && (g_compressionTest);
           cAlgNb++) {
        ...
        if ((g_compressionAlgo != ALL_COMPRESSORS) &&
            (g_compressionAlgo != cAlgNb))
          continue;
        ...
        case 41:
          ...
          LZ4_loadDictHC(&LZ4_streamHC, chunkP[0].origBuffer,
                         chunkP[0].origSize);
          break;
        default:
        ...
        for (loopNb = 1; loopNb <= g_nbIterations; loopNb++) {
          ...
          PROGRESS("%2i-%-34.34s :%10i ->\r", loopNb, compressorName,
                   (int)benchedSize);
          {
            size_t i; /* warming up memory */
          ...
          PROGRESS("%2i-%-34.34s :%10i ->%9i (%5.2f%%),%7.1f MB/s\r", loopNb,
                   compressorName, (int)benchedSize, (int)cSize, ratio,
      ...
      for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
        chunkP[chunkNb].compressedSize = LZ4_compress_default(
            chunkP[chunkNb].origBuffer, chunkP[chunkNb].compressedBuffer,
      ...
      for (dAlgNb = 0;
           (dAlgNb <= NB_DECOMPRESSION_ALGORITHMS) && g_decompressionTest;
           dAlgNb++) {
        ...
        if ((g_decompressionAlgo != ALL_DECOMPRESSORS) &&
            (g_decompressionAlgo != dAlgNb))
          continue;
        ...
        case 12:
          if (dAlgNb == 10) {
            decompressionFunction = local_LZ4F_decompress; /* can be skipped */
            dName = "LZ4F_decompress";
          }
          if (dAlgNb == 11) {
            decompressionFunction =
                local_LZ4F_decompress_followHint; /* can be skipped */
            dName = "LZ4F_decompress_followHint";
          }
          if (dAlgNb == 12) {
            decompressionFunction =
                local_LZ4F_decompress_noHint; /* can be skipped */
            dName = "LZ4F_decompress_noHint";
          }
          /* prepare compressed data using frame format */
          {
            size_t const fcsize =
                LZ4F_compressFrame(compressed_buff, (size_t)compressedBuffSize,
        ...
        {
          size_t i; /* zeroing source area, for CRC checking */
          for (i = 0; i < benchedSize; i++)
            orig_buff[i] = 0;
        }
        ...
        for (loopNb = 1; loopNb <= g_nbIterations; loopNb++) {
          ...
          while (BMK_GetClockSpan(clockTime) < TIMELOOP) {
            for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
              int const decodedSize = decompressionFunction(
          ...
          PROGRESS("%2i-%-34.34s :%10i -> %7.1f MB/s\r", loopNb, dName,
                   (int)benchedSize, (double)benchedSize / bestTime / 1000000);
