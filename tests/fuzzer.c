 ...
 **************************************/
#if defined(__unix__) &&                                                       \
    !defined(_AIX)    /* must be included before platform.h for MAP_ANONYMOUS */
...
typedef size_t uintptr_t; /* true on most systems, except OpenVMS-64 (which
                             doesn't need address overflow test) */
#endif
...
#define FUZ_RANDLENGTH                                                         \
  (((FUZ_rand(seed) >> 7) & 3) ? (FUZ_rand(seed) % 15)                         \
                               : (FUZ_rand(seed) % 510) + 15)
static void FUZ_fillCompressibleNoiseBuffer(void *buffer, size_t bufferSize,
                                            double proba, U32 *seed) {
...
static int FUZ_AddressOverflow(void) {
  ...
  for (nbBuff = 2; nbBuff < MAX_NB_BUFF_I134; nbBuff++) {
    ...
    if (((uintptr_t)buffers[nbBuff] > (uintptr_t)0x80000000) &&
        (!highAddress)) {
...

    {
      size_t const sizeToGenerateOverflow =
          (size_t)(-((uintptr_t)buffers[nbBuff - 1]) + 512);
...
static void *FUZ_createLowAddr(size_t size) {
  void *const lowBuff = mmap((void *)(0x1000), size, PROT_READ | PROT_WRITE,
...
static void *FUZ_createLowAddr(size_t size) { return malloc(size); }
...
static int FUZ_test(U32 seed, U32 nbCycles, const U32 startCycle,
                    const double compressibility, U32 duration_s) {
  ...
  size_t const compressedBufferSize =
      (size_t)LZ4_compressBound(FUZ_MAX_BLOCK_SIZE);
  char *const compressedBuffer = (char *)malloc(compressedBufferSize);
  char *const decodedBuffer =
      (char *)malloc(FUZ_MAX_DICT_SIZE + FUZ_MAX_BLOCK_SIZE);
...
#define FUZ_DISPLAYTEST(...)                                                   \
    ...
    abort();
  {
    ...
    FUZ_fillCompressibleNoiseBuffer(CNBuffer, COMPRESSIBLE_NOISE_LENGTH,
                                    compressibility, &randState);
  }
  ...
  for (cycleNb = startCycle;
    ...
    int const blockStart =
        (int)(FUZ_rand(&randState) %
              (U32)(COMPRESSIBLE_NOISE_LENGTH - blockSize - 1)) +
        1;
    ...
    FUZ_DISPLAYTEST("test LZ4_compress_destSize()");
    {
      ...
      int const targetSize =
          srcSize * (int)((FUZ_rand(&randState) & 127) + 1) >> 7;
      char const endCheck = (char)(FUZ_rand(&randState) & 255);
      compressedBuffer[targetSize] = endCheck;
      cSize =
          LZ4_compress_destSize(block, compressedBuffer, &srcSize, targetSize);
      FUZ_CHECKTEST(cSize > targetSize,
      ...
      DISPLAYLEVEL(5, "destSize : %7i/%7i; content%7i/%7i ", cSize, targetSize,
                   srcSize, blockSize);
      if (targetSize > 0) {
        /* check correctness */
        ...
        {
          int const dSize = LZ4_decompress_safe(compressedBuffer, decodedBuffer,
                                                cSize, srcSize);
          FUZ_CHECKTEST(dSize < 0, "LZ4_decompress_safe() failed on data "
                                   "compressed by LZ4_compress_destSize");
          FUZ_CHECKTEST(
              dSize != srcSize,
              "LZ4_decompress_safe() failed : did not fully decompressed data");
        }
        FUZ_CHECKTEST(decodedBuffer[srcSize] != canary,
                      "LZ4_decompress_safe() overwrite dst buffer !");
        {
          U32 const crcDec = XXH32(decodedBuffer, (size_t)srcSize, 0);
          FUZ_CHECKTEST(crcDec != crcBase,
    ...
    FUZ_DISPLAYTEST("test LZ4_compress_HC_destSize()");
    {
      ...
      compressedBuffer[targetSize] = endCheck;
      cSize = LZ4_compress_HC_destSize(ctx, block, compressedBuffer, &srcSize,
      ...
      FUZ_CHECKTEST(srcSize > blockSize,
                    "LZ4_compress_HC_destSize() fed more than src buffer !");
      if (targetSize > 0) {
        /* check correctness */
        U32 const crcBase = XXH32(block, (size_t)srcSize, 0);
        char const canary = (char)(FUZ_rand(&randState) & 255);
        ...
        decodedBuffer[srcSize] = canary;
        {
          ...
          FUZ_CHECKTEST(dSize < 0,
                        "LZ4_decompress_safe failed (%i) on data compressed by "
                        "LZ4_compressHC_destSize",
                        dSize);
          FUZ_CHECKTEST(dSize != srcSize,
                        "LZ4_decompress_safe failed : decompressed %i bytes, "
                        "was supposed to decompress %i bytes",
                        dSize, srcSize);
        }
        FUZ_CHECKTEST(decodedBuffer[srcSize] != canary,
                      "LZ4_decompress_safe overwrite dst buffer !");
    ...
    HCcompressedSize =
        LZ4_compress_HC(block, compressedBuffer, blockSize,
    ...
    {
      int const r = LZ4_compress_HC_extStateHC(
          stateLZ4HC, block, compressedBuffer, blockSize,
          (int)compressedBufferSize, compressionLevel);
    ...
    {
      int const r = LZ4_compress_HC_extStateHC_fastReset(
          stateLZ4HC, block, compressedBuffer, blockSize,
          (int)compressedBufferSize, compressionLevel);
    ...
    {
      int const r =
          LZ4_compress_fast_extState(stateLZ4, block, compressedBuffer,
                                     blockSize, (int)compressedBufferSize, 8);
    ...
    FUZ_DISPLAYTEST();
    {
      int const r = LZ4_compress_fast_extState_fastReset(
          stateLZ4, block, compressedBuffer, blockSize,
    ...
    compressedSize = LZ4_compress_default(block, compressedBuffer, blockSize,
                                          (int)compressedBufferSize);
    FUZ_CHECKTEST(compressedSize <= 0, "LZ4_compress_default() failed");

    /* Decompression tests */

    /* Test decompress_fast() with input buffer size exactly correct => must not
     * read out of bound */
    {
      ...
      {
        int const r =
            LZ4_decompress_fast(cBuffer_exact, decodedBuffer, blockSize);
        FUZ_CHECKTEST(r < 0,
                      "LZ4_decompress_fast failed despite correct space");
        FUZ_CHECKTEST(
            r != compressedSize,
            "LZ4_decompress_fast failed : did not fully read compressed data");
      }
      {
        U32 const crcCheck = XXH32(decodedBuffer, (size_t)blockSize, 0);
        FUZ_CHECKTEST(crcCheck != crcOrig,
      ...
      {
        int const r =
            LZ4_decompress_fast(cBuffer_exact, decodedBuffer, blockSize - 1);
        FUZ_CHECKTEST(r >= 0, "LZ4_decompress_fast should have failed, due to "
                              "Output Size being too small");
      }
      FUZ_CHECKTEST(decodedBuffer[blockSize - 1] != 0,
      ...
      {
        int const r = LZ4_decompress_safe(cBuffer_exact, decodedBuffer,
                                          compressedSize, blockSize);
        FUZ_CHECKTEST(r < 0,
                      "LZ4_decompress_safe failed despite sufficient space");
        FUZ_CHECKTEST(r != blockSize,
      ...
      {
        int const r = LZ4_decompress_safe(cBuffer_exact, decodedBuffer,
                                          compressedSize, blockSize + 1);
        FUZ_CHECKTEST(
            r < 0, "LZ4_decompress_safe failed despite amply sufficient space");
        FUZ_CHECKTEST(r != blockSize,
                      "LZ4_decompress_safe did not regenerate original data");
      }
      ...
      decodedBuffer[blockSize - 1] = 0;
      {
        int const r = LZ4_decompress_safe(cBuffer_exact, decodedBuffer,
                                          compressedSize, blockSize - 1);
        FUZ_CHECKTEST(r >= 0, "LZ4_decompress_safe should have failed, due to "
      ...
      if (blockSize > 10) {
        ...
        {
          int const r = LZ4_decompress_safe(cBuffer_exact, decodedBuffer,
                                            compressedSize, blockSize - 10);
          FUZ_CHECKTEST(r >= 0, "LZ4_decompress_safe should have failed, due "
                                "to Output Size being 10 bytes too short");
        }
        FUZ_CHECKTEST(
            decodedBuffer[blockSize - 10],
            "LZ4_decompress_safe overrun specified output buffer size");
      }
        ...
        for (;;) {
          ...
          /* add noise */
          {
            ...
            size_t const noiseLength =
                MIN(rNoiseLength, (size_t)compressedSize - pos);
            size_t const noiseStart = FUZ_rand(&randState) %
                                      (COMPRESSIBLE_NOISE_LENGTH - noiseLength);
            memcpy(cBuffer_exact + pos, (const char *)CNBuffer + noiseStart,
                   noiseLength);
            pos += noiseLength;
          }
        }
      ...
      FUZ_DISPLAYTEST("decompress noisy source ");
      {
        ...
        {
          int const decompressResult = LZ4_decompress_safe(
              cBuffer_exact, decodedBuffer, compressedSize, blockSize);
          ...
          FUZ_CHECKTEST(decompressResult > blockSize,
                        "LZ4_decompress_safe on noisy src : result is too "
                        "large : %u > %u (dst buffer)",
    ...
    FUZ_DISPLAYTEST();
    {
      int const r = LZ4_decompress_safe(compressedBuffer, decodedBuffer,
                                        compressedSize - 1, blockSize);
      FUZ_CHECKTEST(
          r >= 0,
          "LZ4_decompress_safe should have failed, due to input size being one "
          "byte too short (blockSize=%i, result=%i, compressedSize=%i)",
    ...
    FUZ_DISPLAYTEST("test LZ4_decompress_safe_partial");
    {
      ...
      char const sentinel = decodedBuffer[targetSize] =
          block[targetSize] ^ 0x5A;
      int const decResult = LZ4_decompress_safe_partial(
          compressedBuffer, decodedBuffer, inCSize, targetSize, blockSize);
      FUZ_CHECKTEST(decResult < 0,
                    "LZ4_decompress_safe_partial failed despite valid input "
                    "data (error:%i)",
                    decResult);
      FUZ_CHECKTEST(decResult != targetSize,
                    "LZ4_decompress_safe_partial did not regenerated required "
                    "amount of data (%i < %i <= %i)",
                    decResult, targetSize, blockSize);
      FUZ_CHECKTEST(decodedBuffer[targetSize] != sentinel,
                    "LZ4_decompress_safe_partial overwrite beyond requested "
                    "size (though %i <= %i <= %i)",
                    decResult, targetSize, blockSize);
      FUZ_CHECKTEST(memcmp(block, decodedBuffer, (size_t)targetSize),
                    "LZ4_decompress_safe_partial: corruption detected in "
                    "regenerated data");
    }
    ...
    ret = LZ4_compress_default(block, compressedBuffer, blockSize,
                               compressedSize);
    FUZ_CHECKTEST(ret == 0,
                  "LZ4_compress_default() failed despite sufficient space");

    /* Test compression with output size being exactly what's necessary and
     * external state (should work) */
    FUZ_DISPLAYTEST("test LZ4_compress_fast_extState() with output buffer just "
                    "the right size");
    ret = LZ4_compress_fast_extState(stateLZ4, block, compressedBuffer,
                                     blockSize, compressedSize, 1);
    FUZ_CHECKTEST(
        ret == 0,
    ...
    ret = LZ4_compress_HC(block, compressedBuffer, blockSize, HCcompressedSize,
                          compressionLevel);
    FUZ_CHECKTEST(ret == 0,
    ...
    /* Test HC compression with output size being exactly what's necessary
     * (should work) */
    FUZ_DISPLAYTEST("test LZ4_compress_HC_extStateHC() with output buffer just "
                    "the right size");
    ret = LZ4_compress_HC_extStateHC(stateLZ4HC, block, compressedBuffer,
                                     blockSize, HCcompressedSize,
                                     compressionLevel);
    FUZ_CHECKTEST(
        ret == 0,
        ...
        "test LZ4_compress_default() with output buffer a bit too short");
    {
      ...
      {
        int const cSize = LZ4_compress_default(
            block, compressedBuffer, blockSize, compressedSize - missingBytes);
        FUZ_CHECKTEST(cSize,
                      "LZ4_compress_default should have failed (output buffer "
                      "too small by %i byte)",
                      missingBytes);
      }
      FUZ_CHECKTEST(
          compressedBuffer[compressedSize - missingBytes],
          "LZ4_compress_default overran output buffer ! (%i missingBytes)",
          missingBytes)
    }
        ...
        "test LZ4_compress_HC() with output buffer a bit too short");
    {
      ...
      compressedBuffer[HCcompressedSize - missingBytes] = 0;
      {
        int const hcSize =
            LZ4_compress_HC(block, compressedBuffer, blockSize,
                            HCcompressedSize - missingBytes, compressionLevel);
        FUZ_CHECKTEST(hcSize,
                      "LZ4_compress_HC should have failed (output buffer too "
                      "small by %i byte)",
                      missingBytes);
      }
      FUZ_CHECKTEST(compressedBuffer[HCcompressedSize - missingBytes],
                    "LZ4_compress_HC overran output buffer ! (%i missingBytes)",
                    missingBytes)
    }
    ...
    FUZ_DISPLAYTEST(
        "test LZ4_compress_fast_continue() with dictionary of size %i",
        dictSize);
    {
      LZ4_stream_t LZ4_stream;
      LZ4_initStream(&LZ4_stream, sizeof(LZ4_stream));
      LZ4_compress_fast_continue(&LZ4_stream, dict, compressedBuffer, dictSize,
                                 (int)compressedBufferSize,
                                 1); /* Just to fill hash tables */
      blockContinueCompressedSize =
          LZ4_compress_fast_continue(&LZ4_stream, block, compressedBuffer,
                                     blockSize, (int)compressedBufferSize, 1);
      FUZ_CHECKTEST(blockContinueCompressedSize == 0,
    ...
    ret = LZ4_decompress_fast_usingDict(compressedBuffer,
                                        decodedBuffer + dictSize, blockSize,
                                        decodedBuffer, dictSize);
    FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                  "LZ4_decompress_fast_usingDict did not read all compressed "
                  "block input");
    {
      U32 const crcCheck =
          XXH32(decodedBuffer + dictSize, (size_t)blockSize, 0);
      if (crcCheck != crcOrig) {
        FUZ_findDiff(block, decodedBuffer);
        EXIT_MSG(
            "LZ4_decompress_fast_usingDict corrupted decoded data (dict %i)",
            dictSize);
      }
    }
    ...
    ret = LZ4_decompress_safe_usingDict(
        compressedBuffer, decodedBuffer + dictSize, blockContinueCompressedSize,
    ...
    dict -= (size_t)(FUZ_rand(&randState) & 0xF) +
            1; /* create space, so now dictionary is an ExtDict */
    if (dict < (char *)CNBuffer)
      dict = (char *)CNBuffer;
    LZ4_loadDict(&LZ4dictBody, dict, dictSize);
    blockContinueCompressedSize =
        LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer,
                                   blockSize, (int)compressedBufferSize, 1);
    FUZ_CHECKTEST(blockContinueCompressedSize == 0,
                  "LZ4_compress_fast_continue failed");

    FUZ_DISPLAYTEST("LZ4_compress_fast_continue() with dictionary and output "
                    "buffer too short by one byte");
    LZ4_loadDict(&LZ4dictBody, dict, dictSize);
    ret = LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer,
                                     blockSize, blockContinueCompressedSize - 1,
                                     1);
    FUZ_CHECKTEST(ret > 0,
                  "LZ4_compress_fast_continue using ExtDict should fail : one "
                  "missing byte for output buffer : %i written, %i buffer",
                  ret, blockContinueCompressedSize);

    FUZ_DISPLAYTEST("test LZ4_compress_fast_continue() with dictionary loaded "
                    "with LZ4_loadDict()");
    DISPLAYLEVEL(5,
                 " compress %i bytes from buffer(%p) into dst(%p) using "
                 "dict(%p) of size %i \n",
                 blockSize, (const void *)block, (void *)decodedBuffer,
    ...
    FUZ_CHECKTEST(
        ret != blockContinueCompressedSize,
        "LZ4_compress_limitedOutput_compressed size is different (%i != %i)",
        ret, blockContinueCompressedSize);
    FUZ_CHECKTEST(ret <= 0, "LZ4_compress_fast_continue should work : enough "
    ...
    DISPLAYLEVEL(
        5, " decoding %i bytes from buffer(%p) using dict(%p) of size %i \n",
        blockSize, (void *)decodedBuffer, (const void *)dict, dictSize);
    decodedBuffer[blockSize] = 0;
    ret = LZ4_decompress_fast_usingDict(compressedBuffer, decodedBuffer,
                                        blockSize, dict, dictSize);
    FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                  "LZ4_decompress_fast_usingDict did not read all compressed "
                  "block input");
    FUZ_CHECKTEST(
    ...
    FUZ_DISPLAYTEST();
    decodedBuffer[blockSize] = 0;
    ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer,
                                        blockContinueCompressedSize, blockSize,
                                        dict, dictSize);
    FUZ_CHECKTEST(
        ret != blockSize,
        "LZ4_decompress_safe_usingDict did not regenerate original data");
    ...
    decodedBuffer[blockSize - 1] = 0;
    ret = LZ4_decompress_fast_usingDict(compressedBuffer, decodedBuffer,
                                        blockSize - 1, dict, dictSize);
    FUZ_CHECKTEST(ret >= 0, "LZ4_decompress_fast_usingDict should have failed "
                            ": wrong original size (-1 byte)");
    FUZ_CHECKTEST(
        decodedBuffer[blockSize - 1],
        "LZ4_decompress_fast_usingDict overrun specified output buffer size");
    ...
    ret = LZ4_decompress_safe_usingDict(compressedBuffer, decodedBuffer,
                                        blockContinueCompressedSize,
                                        blockSize - 1, dict, dictSize);
    FUZ_CHECKTEST(ret >= 0, "LZ4_decompress_safe_usingDict should have failed "
                            ": not enough output size (-1 byte)");
    FUZ_CHECKTEST(
        decodedBuffer[blockSize - 1],
        "LZ4_decompress_safe_usingDict overrun specified output buffer size");
    ...
    FUZ_DISPLAYTEST();
    {
      ...
      if (blockSize > missingBytes) {
        ...
        ret = LZ4_decompress_safe_usingDict(
            compressedBuffer, decodedBuffer, blockContinueCompressedSize,
            blockSize - missingBytes, dict, dictSize);
        FUZ_CHECKTEST(ret >= 0,
                      "LZ4_decompress_safe_usingDict should have failed : "
                      "output buffer too small (-%i byte)",
                      missingBytes);
        FUZ_CHECKTEST(decodedBuffer[blockSize - missingBytes],
                      "LZ4_decompress_safe_usingDict overrun specified output "
                      "buffer size (-%i byte) (blockSize=%i)",
    ...
    /* Compress using external dictionary stream */
    {
      ...
      FUZ_CHECKTEST(expectedSize <= 0,
                    "LZ4_compress_fast_continue reference compression for "
                    "extDictCtx should have succeeded");
      ...
      FUZ_CHECKTEST(blockContinueCompressedSize != expectedSize,
                    "LZ4_compress_fast_continue using extDictCtx produced "
                    "different-sized output (%d expected vs %d actual)",
                    expectedSize, blockContinueCompressedSize);
      FUZ_CHECKTEST(XXH32(compressedBuffer, (size_t)blockContinueCompressedSize,
                          0) != expectedCrc,
                    "LZ4_compress_fast_continue using extDictCtx produced "
                    "different output");

      FUZ_DISPLAYTEST(
          "LZ4_compress_fast_continue() after LZ4_attach_dictionary(), but "
          "output buffer is 1 byte too short");
      LZ4_resetStream_fast(&LZ4_stream);
      LZ4_attach_dictionary(&LZ4_stream, &LZ4dictBody);
      ret = LZ4_compress_fast_continue(&LZ4_stream, block, compressedBuffer,
                                       blockSize,
                                       blockContinueCompressedSize - 1, 1);
      FUZ_CHECKTEST(
          ret > 0,
          "LZ4_compress_fast_continue using extDictCtx should fail : one "
          "missing byte for output buffer : %i written, %i buffer",
          ret, blockContinueCompressedSize);
      ...
      FUZ_CHECKTEST(
          ...
          ret, blockContinueCompressedSize);
      FUZ_CHECKTEST(ret <= 0,
                    "LZ4_compress_fast_continue using extDictCtx should work : "
      ...
      FUZ_CHECKTEST(
          ret <= 0,
          "LZ4_compress_fast_continue using extDictCtx with re-used context "
          "should work : enough size available within output buffer");
      FUZ_CHECKTEST(ret != expectedSize,
                    "LZ4_compress_fast_continue using extDictCtx produced "
                    "different-sized output");
      FUZ_CHECKTEST(XXH32(compressedBuffer, (size_t)ret, 0) != expectedCrc,
    ...
    FUZ_CHECKTEST(LZ4dictHC->internal_donotuse.dirty,
                  "Context should be clean");

    FUZ_DISPLAYTEST("LZ4_compress_HC_continue with same external dictionary, "
                    "but output buffer 1 byte too short");
    LZ4_loadDictHC(LZ4dictHC, dict, dictSize);
    ret = LZ4_compress_HC_continue(LZ4dictHC, block, compressedBuffer,
                                   blockSize, blockContinueCompressedSize - 1);
    FUZ_CHECKTEST(ret > 0,
                  "LZ4_compress_HC_continue using ExtDict should fail : one "
                  "missing byte for output buffer (expected %i, but result=%i)",
                  blockContinueCompressedSize, ret);
    /* note : context is no longer dirty after a failed compressed block */

    FUZ_DISPLAYTEST("LZ4_compress_HC_continue with same external dictionary, "
                    "and output buffer exactly the right size");
    LZ4_loadDictHC(LZ4dictHC, dict, dictSize);
    ret = LZ4_compress_HC_continue(LZ4dictHC, block, compressedBuffer,
                                   blockSize, blockContinueCompressedSize);
    FUZ_CHECKTEST(
        ret != blockContinueCompressedSize,
        "LZ4_compress_HC_continue size is different : ret(%i) != expected(%i)",
        ret, blockContinueCompressedSize);
    FUZ_CHECKTEST(ret <= 0, "LZ4_compress_HC_continue should work : enough "
                            "size available within output buffer");
    FUZ_CHECKTEST(LZ4dictHC->internal_donotuse.dirty,
                  "Context should be clean");
    ...
    FUZ_DISPLAYTEST();
    {
      ...
      blockContinueCompressedSize =
          LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer,
                                   blockSize, (int)compressedBufferSize);
      FUZ_CHECKTEST(blockContinueCompressedSize == 0,
                    "LZ4_compress_HC_continue with ExtDictCtx failed");
      FUZ_CHECKTEST(LZ4_streamHC->internal_donotuse.dirty,
                    "Context should be clean");
      ...
      ret =
          LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer,
                                   blockSize, blockContinueCompressedSize - 1);
      FUZ_CHECKTEST(ret > 0,
                    "LZ4_compress_HC_continue using ExtDictCtx should fail : "
                    "one missing byte for output buffer (%i != %i)",
                    ret, blockContinueCompressedSize);
      /* note : context is no longer dirty after a failed compressed block */
      ...
      LZ4_resetStreamHC_fast(LZ4_streamHC, compressionLevel);
      LZ4_attach_HC_dictionary(LZ4_streamHC, LZ4dictHC);
      ret = LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer,
                                     blockSize, blockContinueCompressedSize);
      FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                    "LZ4_compress_HC_continue using ExtDictCtx size is "
                    "different (%i != %i)",
                    ret, blockContinueCompressedSize);
      FUZ_CHECKTEST(ret <= 0,
                    "LZ4_compress_HC_continue using ExtDictCtx should work : "
                    "enough size available within output buffer");
      FUZ_CHECKTEST(LZ4_streamHC->internal_donotuse.dirty,
                    "Context should be clean");
      ...
      ret = LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer,
                                     blockSize, blockContinueCompressedSize);
      FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                    "LZ4_compress_HC_continue using ExtDictCtx and fast reset "
                    "size is different (%i != %i)",
                    ret, blockContinueCompressedSize);
      FUZ_CHECKTEST(ret <= 0,
                    "LZ4_compress_HC_continue using ExtDictCtx and fast reset "
                    "should work : enough size available within output buffer");
    ...
    FUZ_DISPLAYTEST();
    {
      int const availableSpace =
          (int)(FUZ_rand(&randState) % (U32)blockSize) + 5;
      int consumedSize = blockSize;
      FUZ_DISPLAYTEST();
      LZ4_loadDictHC(LZ4dictHC, dict, dictSize);
      LZ4_setCompressionLevel(LZ4dictHC, compressionLevel);
      blockContinueCompressedSize = LZ4_compress_HC_continue_destSize(
          LZ4dictHC, block, compressedBuffer, &consumedSize, availableSpace);
      DISPLAYLEVEL(5,
                   " LZ4_compress_HC_continue_destSize : compressed %6i/%6i "
                   "into %6i/%6i at cLevel=%i \n",
                   consumedSize, blockSize, blockContinueCompressedSize,
                   availableSpace, compressionLevel);
      FUZ_CHECKTEST(blockContinueCompressedSize == 0,
      ...
      FUZ_CHECKTEST(consumedSize > blockSize,
      ...
      FUZ_CHECKTEST(
          ret != consumedSize,
          "LZ4_decompress_safe_usingDict regenerated %i bytes (%i expected)",
          ret, consumedSize);
...
static void FUZ_unitTests(int compressionLevel) {
  ...
  DISPLAYLEVEL(3, "LZ4_decompress_safe() with shortcut edge case \n");
  {
    ...
    *tmp = (char)0xEE;
    ...
    {
      int const r =
          LZ4_decompress_safe(tmp, testVerify, sizeof(tmp), testInputSize);
  ...
  DISPLAYLEVEL(3, "in-place compression using LZ4_compress_default() :");
  {
    ...
    /* compress in-place */
    cSize =
        LZ4_compress_default(startInput, testCompressed, sampleSize, maxCSize);
    ...
    {
      int const dSize =
          LZ4_decompress_safe(testCompressed, testVerify, cSize, testInputSize);
  ...
  DISPLAYLEVEL(3, "in-place decompression, limit case:");
  {
    ...
    memset(
        testInput, 0,
        267); /* calculated exactly so that compressedSize == originalSize-1 */

    {
      XXH64_hash_t const crcOrig = XXH64(testInput, sampleSize, 0);
      int const cSize = LZ4_compress_default(testInput, testCompressed,
                                             sampleSize, testCompressedSize);
      assert(cSize ==
             sampleSize - 1); /* worst case for in-place decompression */

      {
        ...
        {
          int const dSize =
              LZ4_decompress_safe(startInput, testVerify, cSize, sampleSize);
          assert(dSize == sampleSize); /* correct size */
          {
  ...
  DISPLAYLEVEL(3, "LZ4_initStream with multiple valid alignments : ");
  {
    ...
    DISPLAYLEVEL(4,
                 ...
                 (void *)&(shc->state1), (void *)&(shc->state2),
                 (void *)&(shc->state3), (unsigned)sizeof(LZ4_stream_t));
    FUZ_CHECKTEST(LZ4_initStream(&(shc->state1), sizeof(shc->state1)) == NULL,
                  "state1 (%p) failed init", (void *)&(shc->state1));
    FUZ_CHECKTEST(LZ4_initStream(&(shc->state2), sizeof(shc->state2)) == NULL,
                  "state2 (%p) failed init", (void *)&(shc->state2));
    FUZ_CHECKTEST(LZ4_initStream(&(shc->state3), sizeof(shc->state3)) == NULL,
                  "state3 (%p) failed init", (void *)&(shc->state3));
    FUZ_CHECKTEST(
        LZ4_initStream((char *)&(shc->state1) + 1, sizeof(shc->state1)) != NULL,
        "hc1+1 (%p) init must fail, due to bad alignment",
        (void *)((char *)&(shc->state1) + 1));
    free(shc);
  }
  ...
  /* LZ4 streaming tests */
  {
    ...
    {
      int const cs = LZ4_compress_fast_continue(
          &streamingState, testInput, testCompressed, testCompressedSize,
          testCompressedSize - 1, 1);
      FUZ_CHECKTEST(cs == 0,
                    "LZ4_compress_fast_continue() compression failed!");
      {
        int const r = LZ4_decompress_safe(testCompressed, testVerify, cs,
                                          testCompressedSize);
        FUZ_CHECKTEST(r != (int)testCompressedSize,
                      "LZ4_decompress_safe() decompression failed");
      }
    }
    {
    ...
    DISPLAYLEVEL(3, "saveDict (right after init) : ");
    {
      ...
      /* Check access violation with asan */
      {
        ...
        FUZ_CHECKTEST(LZ4_saveDict(ctx, tmp_buffer, sizeof(tmp_buffer)) != 0,
                      "LZ4_saveDict() can't save anything since compression "
                      "hasn't started");
      }
    }
    DISPLAYLEVEL(3, "OK \n");
    ...
    /* ring buffer test */
    {
      ...
      while (iNext + messageSize < testCompressedSize) {
        ...
        compressedSize = LZ4_compress_fast_continue(
            &streamingState, ringBuffer + rNext, testCompressed,
            (int)messageSize, testCompressedSize - ringBufferSize, 1);
        FUZ_CHECKTEST(compressedSize == 0,
                      "LZ4_compress_fast_continue() compression failed");

        {
          int const r = LZ4_decompress_safe_continue(
              &decodeStateSafe, testCompressed, testVerify + dNext,
        ...
        {
          int const r = LZ4_decompress_fast_continue(
              &decodeStateFast, testCompressed, testVerify + dNext,
              (int)messageSize);
          FUZ_CHECKTEST(
              r != compressedSize,
              "ringBuffer : LZ4_decompress_fast_continue() test failed");
        }
  ...
  DISPLAYLEVEL(3, "LZ4_initStreamHC with multiple valid alignments : ");
  {
    ...
    assert(shc != NULL);
    memset(shc, 0, sizeof(*shc));
    DISPLAYLEVEL(4, "hc1(%p) hc2(%p) hc3(%p) size(0x%x): ", (void *)&(shc->hc1),
                 (void *)&(shc->hc2), (void *)&(shc->hc3),
                 (unsigned)sizeof(LZ4_streamHC_t));
    FUZ_CHECKTEST(LZ4_initStreamHC(&(shc->hc1), sizeof(shc->hc1)) == NULL,
                  "hc1 (%p) failed init", (void *)&(shc->hc1));
    FUZ_CHECKTEST(LZ4_initStreamHC(&(shc->hc2), sizeof(shc->hc2)) == NULL,
                  "hc2 (%p) failed init", (void *)&(shc->hc2));
    FUZ_CHECKTEST(LZ4_initStreamHC(&(shc->hc3), sizeof(shc->hc3)) == NULL,
                  "hc3 (%p) failed init", (void *)&(shc->hc3));
    FUZ_CHECKTEST(LZ4_initStreamHC((char *)&(shc->hc1) + 1, sizeof(shc->hc1)) !=
                      NULL,
                  "hc1+1 (%p) init must fail, due to bad alignment",
                  (void *)((char *)&(shc->hc1) + 1));
    free(shc);
  ...
  /* LZ4 HC streaming tests */
  {
    ...
    DISPLAYLEVEL(3, "Simple HC round-trip : ");
    {
      ...
      result =
          LZ4_compress_HC_continue(&sHC, testInput, testCompressed,
                                   testCompressedSize, testCompressedSize - 1);
      FUZ_CHECKTEST(
          result == 0,
      ...
      result = LZ4_decompress_safe(testCompressed, testVerify, result,
                                   testCompressedSize);
    ...
    DISPLAYLEVEL(3, "saveDictHC test #926 : ");
    {
      ...
      /* Check access violation with asan */
      {
        ...
        FUZ_CHECKTEST(LZ4_saveDictHC(ctx, tmp_buffer, sizeof(tmp_buffer)) != 0,
                      "LZ4_saveDictHC() can't save anything since compression "
                      "hasn't started");
      }
    }
    DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "Long sequence HC_destSize test : ");
    {
      ...
      size_t const targetSize =
          4116; /* size carefully selected to trigger an overflow */
      ...
      result =
          LZ4_compress_HC_destSize(&sHC, (const char *)block, (char *)dstBlock,
                                   &srcSize, (int)targetSize, 3);
      ...
      LZ4_resetStreamHC_fast(
          &sHC, 3); /* make sure the context is clean after the test */
      free(block);
      free(dstBlock);
    }
    DISPLAYLEVEL(3, " OK \n");
    ...
    DISPLAYLEVEL(3, "HC dictionary compression test : ");
    {
      ...
      {
        int const cSize = LZ4_compress_HC_continue(
            &sHC, testInput + 64 KB, testCompressed, testCompressedSize,
            testCompressedSize - 1);
        FUZ_CHECKTEST(cSize == 0,
                      "LZ4_compressHC_limitedOutput_continue() dictionary "
                      "compression failed : @return = %i",
                      cSize);
        FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
        {
          int const dSize = LZ4_decompress_safe_usingDict(
              testCompressed, testVerify, cSize, testCompressedSize, testInput,
              64 KB);
          FUZ_CHECKTEST(dSize != (int)testCompressedSize,
                        "LZ4_decompress_safe() simple dictionary decompression "
                        "test failed");
        }
      }
      {
    ...
    /* multiple HC compression test with dictionary */
    {
      ...
      XXH64_hash_t const crc64 =
          ((void)assert((unsigned)segSize + testCompressedSize < testInputSize),
           XXH64(testInput + segSize, testCompressedSize, 0));
      LZ4_resetStreamHC_fast(&sHC, compressionLevel);
      LZ4_loadDictHC(&sHC, testInput, segSize);
      result1 = LZ4_compress_HC_continue(&sHC, testInput + segSize,
                                         testCompressed, segSize, segSize - 1);
      FUZ_CHECKTEST(result1 == 0,
                    "LZ4_compressHC_limitedOutput_continue() dictionary "
                    "compression failed : result = %i",
                    result1);
      FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
      result2 = LZ4_compress_HC_continue(&sHC, testInput + 2 * (size_t)segSize,
                                         testCompressed + result1, segSize,
                                         segSize - 1);
      ...
      result = LZ4_decompress_safe_usingDict(
          testCompressed, testVerify, result1, segSize, testInput, segSize);
      FUZ_CHECKTEST(
          result != segSize,
          "LZ4_decompress_safe() dictionary decompression part 1 failed");
      result = LZ4_decompress_safe_usingDict(testCompressed + result1,
                                             testVerify + segSize, result2,
                                             segSize, testInput, 2 * segSize);
      FUZ_CHECKTEST(
          result != segSize,
    ...
    /* remote dictionary HC compression test */
    {
      ...
      result =
          LZ4_compress_HC_continue(&sHC, testInput + 64 KB, testCompressed,
                                   testCompressedSize, testCompressedSize - 1);
      FUZ_CHECKTEST(result == 0,
                    "LZ4_compressHC_limitedOutput_continue() remote dictionary "
                    "failed : result = %i",
                    result);
      FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");

      result =
          LZ4_decompress_safe_usingDict(testCompressed, testVerify, result,
                                        testCompressedSize, testInput, 32 KB);
      FUZ_CHECKTEST(result != (int)testCompressedSize,
                    "LZ4_decompress_safe_usingDict() decompression failed "
                    "following remote dictionary HC compression test");
      {
        U64 const crcNew = XXH64(testVerify, testCompressedSize, 0);
        FUZ_CHECKTEST(
            crc64 != crcNew,
    ...
    /* multiple HC compression with ext. dictionary */
    {
      ...
      while (segStart + segSize < testInputSize) {
        ...
        result = LZ4_compress_HC_continue(&sHC, testInput + segStart,
                                          testCompressed, (int)segSize,
        ...
        result = LZ4_decompress_safe_usingDict(
            testCompressed, dst, result, (int)segSize, dict, (int)dictSize);
        FUZ_CHECKTEST(result != (int)segSize,
                      "LZ4_decompress_safe_usingDict() dictionary "
                      "decompression part %i failed",
                      (int)segNb);
        XXH64_update(&crcNewState, dst, segSize);
        {
          ...
          FUZ_CHECKTEST(crcOrig != crcNew,
                        "LZ4_decompress_safe_usingDict() part %i corruption",
                        segNb);
        }
    ...
    /* ring buffer test */
    {
      ...
      while (iNext + messageSize < testCompressedSize) {
        ...
        compressedSize = LZ4_compress_HC_continue(
            &sHC, ringBuffer + rNext, testCompressed, (int)messageSize,
            testCompressedSize - ringBufferSize);
        FUZ_CHECKTEST(compressedSize == 0,
                      "LZ4_compress_HC_continue() compression failed");
        FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");

        assert(messageSize < INT_MAX);
        result = LZ4_decompress_safe_continue(&decodeStateSafe, testCompressed,
                                              testVerify + dNext,
                                              compressedSize, (int)messageSize);
        FUZ_CHECKTEST(
            result != (int)messageSize,
            "ringBuffer : LZ4_decompress_safe_continue() test failed");
        ...
        assert(messageSize < INT_MAX);
        result =
            LZ4_decompress_fast_continue(&decodeStateFast, testCompressed,
                                         testVerify + dNext, (int)messageSize);
        FUZ_CHECKTEST(
    ...
    /* This test uses minimum amount of memory required to setup a decoding ring
     * buffer while being unsynchronized with encoder (no assumption done on how
     * the data is encoded, it just follows LZ4 format specification). This size
     * is documented in lz4.h, and is LZ4_decoderRingBufferSize(maxBlockSize).
     */
    {
      XXH64_state_t xxhOrig;
      XXH64_state_t xxhNewSafe, xxhNewFast;
      ...
      char *const ringBufferFast = testVerify + dBufferSize +
                                   1; /* used by LZ4_decompress_fast_continue */
      int iNext = 0;
      int dNext = 0;
      int compressedSize;

      assert(
          (size_t)dBufferSize * 2 + 1 <
...
#define BSIZE1 (dBufferSize - (maxMessageSize - 1))
      ...
      /* first block */
      messageSize = BSIZE1; /* note : we cheat a bit here, in theory no message
                               should be > maxMessageSize. We just want to fill
                               the decoding ring buffer once. */
      XXH64_update(&xxhOrig, testInput + iNext, (size_t)messageSize);
      crcOrig = XXH64_digest(&xxhOrig);

      compressedSize = LZ4_compress_HC_continue(
          &sHC, testInput + iNext, testCompressed, messageSize,
          testCompressedSize - ringBufferSize);
      FUZ_CHECKTEST(compressedSize == 0,
                    "LZ4_compress_HC_continue() compression failed");
      FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
      ...
      result =
          LZ4_decompress_fast_continue(&decodeStateFast, testCompressed,
                                       ringBufferFast + dNext, messageSize);
      FUZ_CHECKTEST(
          result != compressedSize,
          "64K D.ringBuffer : LZ4_decompress_fast_continue() test failed");
      ...
      FUZ_CHECKTEST(dNext + messageSize <= dBufferSize,
                    "Ring buffer test : second message should require "
      ...
      while (totalMessageSize < 9 MB) {
        ...
        result = LZ4_decompress_safe_continue(
            &decodeStateSafe, testCompressed, ringBufferSafe + dNext,
            compressedSize,
            dBufferSize -
                dNext); /* works without knowing messageSize, under assumption
                           that messageSize <= maxMessageSize */
        FUZ_CHECKTEST(
            result != messageSize,
        ...
        XXH64_update(&xxhNewSafe, ringBufferSafe + dNext, (size_t)messageSize);
        {
          ...
          FUZ_CHECKTEST(crcOrig != crcNew,
                        "LZ4_decompress_safe_continue() decompression "
                        "corruption during D.ringBuffer test");
        }
        ...
        FUZ_CHECKTEST(
            result != compressedSize,
            "D.ringBuffer : LZ4_decompress_fast_continue() test failed");
        XXH64_update(&xxhNewFast, ringBufferFast + dNext, (size_t)messageSize);
        {
          U64 const crcNew = XXH64_digest(&xxhNewFast);
          if (crcOrig != crcNew)
          ...
          FUZ_CHECKTEST(crcOrig != crcNew,
                        "LZ4_decompress_fast_continue() decompression "
                        "corruption during D.ringBuffer test");
        }
  ...
  /* encode congenerical sequence test for HC compressors */
  {
    ...
    for (dlen = dst_min_len; dlen <= dst_max_len; dlen += dst_step) {
      ...
      for (slen = src_len - 3; slen <= src_len + 3; slen++) {
        ...
        dsz2 = LZ4_compress_HC_destSize(sHC, sbuf2, dbuf2, &srcsz2, dlen,
                                        compressionLevel);
        DISPLAYLEVEL(
            5, "LZ4_compress_HC_destSize: %i bytes compressed into %i bytes, ",
            srcsz2, dsz2);
        DISPLAYLEVEL(5, "last token : 0x%0X, ", dbuf2[dsz2 - 6]);
        DISPLAYLEVEL(5, "last ML extra lenbyte : 0x%0X, \n", dbuf2[dsz2 - 7]);
        FUZ_CHECKTEST(dbuf2[dlen] != endchk,
                      "LZ4_compress_HC_destSize() overwrite dst buffer !");
        FUZ_CHECKTEST(dsz2 <= 0,
                      "LZ4_compress_HC_destSize() compression failed");
        FUZ_CHECKTEST(
            dsz2 > dlen,
            "LZ4_compress_HC_destSize() result larger than dst buffer !");
        FUZ_CHECKTEST(srcsz2 > slen,
                      "LZ4_compress_HC_destSize() read more than src buffer !");
        FUZ_CHECKTEST(dsz2 != dsz1,
        ...
        FUZ_CHECKTEST(memcmp(dbuf2, dbuf1, (size_t)dsz2),
                      "LZ4_compress_HC_destSize() return incorrect data into "
                      "dst buffer !");

        res2 = LZ4_decompress_safe(dbuf2, sbuf1, dsz2, src_buf_size);
        FUZ_CHECKTEST(res2 != srcsz1,
...
int main(int argc, const char **argv) {
  ...
  {
    int const result =
        FUZ_test(seed, nbTests, testNb, ((double)proba) / 100, duration);
    if (use_pause) {
      DISPLAY("press enter ... \n");
      (void)getchar();
    }
